...
# --- Creation ---
create_mon_env() {
    if conda env list | grep -q "mon"; then
        echo -e "${YELLOW}'mon' environment already exists. Skipping creation.${NC}"
        return 0
    fi

    echo -e "\n${YELLOW}Creating 'mon' virtual environment...${NC}"
    local env_yaml
...
setup_mon_env() {
    if ! conda env list | grep -q "mon"; then
        echo -e "${RED}'mon' environment does not exist. Creating...${NC}"
        create_mon_env
    fi
...
# --- Retrieval ---
list_envs() {
    echo -e "\n${BLUE}Listing Conda environments...${NC}"
    conda env list
}
...
delete_mon_env() {
    if ! conda env list | grep -q "mon"; then
        echo -e "${YELLOW}'mon' environment does not exist. Nothing to delete.${NC}"
        return 0
    fi

    echo -e "\n${BLUE}Deleting 'mon' virtual environment...${NC}"
    conda activate base
    conda remove --name mon --all
    conda clean --all -y
    echo -e "${GREEN}'mon' environment deleted.${NC}"
    conda env list
}
...
manage_mon_env() {
    while true; do
        echo -e ""
        echo -e "${BLUE}================================================================================${NC}"
        echo -e "${BLUE}                             MON ENVIRONMENT MANAGER                            ${NC}"
        echo -e "${BLUE}================================================================================${NC}"
