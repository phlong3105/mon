		...
		for key_img, _ in gth_dict[month].items():
			...
			if key_img in pred_dict[month]:
				if len(pred_dict[month][key_img]["boxes"]) > 0:
					...
					det_boxes.append(np.array(pred_dict[month][key_img]["boxes"]))
					det_scores.append(np.array(pred_dict[month][key_img]["score"]))
					det_classes.append(np.array(pred_dict[month][key_img]["labels"]))
	
	# for month, _ in tqdm(pred_dict.items()):
	# 	for key_img, _ in pred_dict[month].items():
	# 		pass
...
def run_evaluation(opt):
	...
	header = ["Name", "Precision/mAP", "Precision/mAP@.50IOU", "Precision/mAP@.75IOU", "Precision/mAP (small)",
	          "Precision/mAP (medium)",
	          "Precision/mAP (large)", "Recall/AR@1", "Recall/AR@10", "Recall/AR@100", "Recall/AR@100 (small)",
	          "Recall/AR@100 (medium)", "Recall/AR@100 (large)"]
	...
	# 	print(metrics)
	metrics = evaluation_one(gt_file, pred_file, "")
	print(metrics)
...
if __name__ == "__main__":
	...
	parser.add_argument("--pred-file",   default=os.path.join("inference", "predictions_val4.pkl"), type=str, help="Prediction")
