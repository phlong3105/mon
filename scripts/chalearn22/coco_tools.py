...
class COCOEvalWrapper(cocoeval.COCOeval):
	...
	def __init__(
		self, groundtruth=None, detections=None, agnostic_mode=False,
		iou_type='bbox'
		):
...
def ExportSingleImageGroundtruthToCoco(
	...
	if len(groundtruth_classes.shape) != 1:
		raise ValueError(
			'groundtruth_classes is '
			'expected to be of rank 1.'
			)
	if len(groundtruth_boxes.shape) != 2:
		raise ValueError(
			'groundtruth_boxes is expected to be of '
			'rank 2.'
			)
	if groundtruth_boxes.shape[1] != 4:
		raise ValueError(
			'groundtruth_boxes should have '
			'shape[1] == 4.'
			)
...
def ExportSingleImageDetectionBoxesToCoco(
	...
	if len(detection_boxes.shape) != 2:
		raise ValueError(
			f'All entries in detection_boxes expected to be of '
			f'rank 2.'
			)
	if detection_boxes.shape[1] != 4:
		raise ValueError(
			'All entries in detection_boxes should have '
			'shape[1] == 4.'
			)
