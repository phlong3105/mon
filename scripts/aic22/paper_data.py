...
from one import get_image_hw
from one import is_image_file
from one import progress_bar
from one import random_patch_numpy_image_box
from one import read_image
...
image_file_pattern = os.path.join(data_dir, "aicity", "aic22retail", "train", "images", "*")
generate_image_dir = os.path.join(data_dir, "aicity", "aic22retail", "train", "generate_images")
generate_label_dir = os.path.join(data_dir, "aicity", "aic22retail", "train", "generate_labels")
...
background          = read_image("../data/aic22retail/background.png", VisionBackend.CV)
num_items           = 20
num_generate_images = 3  # math.floor(len(image_files) / num_items)

with progress_bar() as pbar:
	item_idx = 0
	for i in pbar.track(range(num_generate_images), description=f"[bright_yellow]Generating files"):
		images   = []
		segments = []
		ids      = []
		...
		cv2.imwrite(f"{i}_result.jpg", result)
