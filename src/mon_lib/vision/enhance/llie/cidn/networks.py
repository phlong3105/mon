...
class E_content_share(nn.Module):
    ...
    def __init__(self, input_dim_a, input_dim_b):
        ...
        enc_share = []
        tch = 64
        enc_share += [LeakyReLUConv2d(input_dim_a, tch, kernel_size=7, stride=1, padding=3)]
        for i in range(1, 3):
            enc_share += [ReLUINSConv2d(tch, tch * 2, kernel_size=3, stride=2, padding=1)]
            tch *= 2
        for i in range(0, 3):
            enc_share += [INSResBlock(tch, tch)]
        ...
        self.convA = nn.Sequential(*enc_share)
        self.convB = nn.Sequential(*enc_share)
...
class E_attr_concat_share(nn.Module):
    ...
    def forward(self, xa, xb):
        ...
        output_A = self.fc_A(conv_flat_A)
        outputVar_A = self.fcVar_A(conv_flat_A)
        x_conv_B = self.conv_A(xb)
        conv_flat_B = x_conv_B.view(xb.size(0), -1)
        output_B = self.fc_A(conv_flat_B)
        outputVar_B = self.fcVar_A(conv_flat_B)
        return output_A, outputVar_A, output_B, outputVar_B
...
class Vgg16(nn.Module):
    ...
    def forward(self, X, opt):
        ...
        if opt.vgg_choose == "conv4_3":
            ...
            return relu5_2
        elif opt.vgg_choose == "relu5_3":
            return relu5_3
        elif opt.vgg_choose == "relu3_1":
            return relu3_1
        elif opt.vgg_choose == "relu2_2":
            return relu2_2
...
def load_vgg16(model_dir, gpu):
    ...
    # vgg = torch.nn.DataParallel(vgg, gpu)
    return vgg
