...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640):
        ...
        videos = [x for x in files if
                  os.path.splitext(x)[-1].lower() in vid_formats]
...
class LoadWebcam:  # for inference
    def __init__(self, pipe=0, img_size=640):
        ...
        # pipe = 'rtsp://username:password@192.168.1.64/1'  # IP camera with
        # login
        # pipe = 'rtsp://170.93.143.139/rtplive
        # /470011e600ef003a004ee33696235daa'  # IP traffic camera
        # pipe = 'http://wmccpinetop.axiscam.net/mjpg/video.mjpg'  # IP golf
        # camera
        
        # https://answers.opencv.org/question/215996/changing-gstreamer
        ...
        # https://answers.opencv.org/question/200787/video-acceleration
        # -gstremer-pipeline-in-videocapture/
        # https://stackoverflow.com/questions/54095699/install-gstreamer
        # -support-for-opencv-python-package  # install help
        # pipe = "rtspsrc location=rtsp://root:root@192.168.0.91:554/axis
        # -media/media.amp?videocodec=h264&resolution=3840x2160
    ...
    def __init__(self, sources='streams.txt', img_size=640):
        ...
        if os.path.isfile(sources):
            with open(sources, 'r') as f:
                sources = [x.strip() for x in f.read().splitlines() if
                           len(x.strip())]
        else:
            sources = [sources]
        ...
        if not self.rect:
            print(
                'WARNING: Different stream shapes detected. For optimal '
                'performance supply similarly-shaped streams.'
            )
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        # Check cache
        cache_path = str(
            Path(self.label_files[0]).parent
        ) + '.cache'  # cached labels
        ...
        # Cache labels
        create_datasubset, extract_bounding_boxes, labels_loaded = False, \
            False, False
        nm, nf, ne, ns, nd = 0, 0, 0, 0, 0  # number missing, found, empty,
        # datasubset, duplicate
        ...
        for i, file in enumerate(pbar):
            ...
            if l.shape[0]:
                ...
                assert (l[:,
                        1:] <= 1).all(), 'non-normalized or out of bounds ' \
                                         'coordinate labels: %s' % file
                ...
                ne += 1  # print('empty labels for image %s' %
                # self.img_files[i])  # file empty
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        if r != 1:  # always resize down, only resize up if training with
            # augmentation
            interp = cv2.INTER_AREA if r < 1 and not self.augment else \
                cv2.INTER_LINEAR
            img = cv2.resize(
                img,
                (int(w0 * r), int(h0 * r)),
...
def load_mosaic(self, index):
    ...
    if len(labels4):
        ...
        # np.clip(labels4[:, 1:] - s / 2, 0, s, out=labels4[:, 1:])  # use
        # with center crop
        np.clip(
            labels4[:, 1:],
            0,
    ...
    # Augment
    # img4 = img4[s // 2: int(s * 1.5), s // 2:int(s * 1.5)]  # center crop (
    # WARNING, requires box pruning)
...
def random_perspective(
    ...
    if n:
        ...
        x = xy[:, [0, 2, 4, 6]]
        y = xy[:, [1, 3, 5, 7]]
        xy = np.concatenate((x.min(1), y.min(1), x.max(1), y.max(1))).reshape(
            4,
            n
        ).T
        ...
        # xy = np.concatenate((x - w / 2, y - h / 2, x + w / 2, y + h /
        # 2)).reshape(4, n).T
...
def box_candidates(
    ...
    # Compute candidate boxes: bbox1 before augment, bbox2 after augment,
...
def cutout(image, labels):
    ...
    def bbox_ioa(box1, box2):
        # Returns the intersection over bbox2 area given bbox1, bbox2. bbox1
        # is 4, bbox2 is nx4. boxes are x1y1x2y2
        box2 = box2.transpose()
...
def recursive_dataset2bmp(
    ...
    formats = [x.lower() for x in img_formats] + [x.upper() for x in
                                                  img_formats]
