...
__all__ = [
    "AICVehicleCountingCamera",
]


# MARK: - AICVehicleCountingCamera

@CAMERAS.register(name="aic21_vehicle_counting_camera")
class AICVehicleCountingCamera(BaseCamera):
    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        Args:
            data_dir (str):
                Location that stores all datasets.
            dataset (str):
    ...
    def init_moving_object(self):
        ...
        Vehicle.min_traveled_distance = cfg["min_traveled_distance"]
        Vehicle.min_entering_distance = cfg["min_entering_distance"]
        Vehicle.min_hit_streak        = cfg["min_hit_streak"]
        Vehicle.max_age               = cfg["max_age"]
    ...
    def init_result_writer(self, result_writer: Union[AIC21VehicleCountingWriter, dict]):
        """Initialize data writer.
        ...
        Args:
            result_writer (AICCountingWriter, dict):
                Result writer object or a result writer's config dictionary.
        """
        if isinstance(result_writer, AIC21VehicleCountingWriter):
            ...
            self.result_writer           = AIC21VehicleCountingWriter(**result_writer)
        ...
        with progress_bar() as pbar:
            for images, indexes, files, rel_paths in pbar.track(
                ...
                for idx, detections in enumerate(batch_detections):
                    ...
                    in_roi_mos = [o for o in self.mos if (o.is_confirmed or o.is_counting or o.is_to_be_counted)]
                    MOI.associate_moving_objects_to_mois(
                        objs=in_roi_mos, mois=self.mois, shape_type="polygon"
                    )
                    to_be_counted_mos = [o for o in in_roi_mos if (o.is_to_be_counted and o.is_countable is False)]
                    MOI.associate_moving_objects_to_mois(
