...
def isqrt_newton_schulz_autograd(A, numIters):
	...
	normA=A.norm()
	Y = A.div(normA)
	I = torch.eye(dim,dtype=A.dtype,device=A.devices)
	Z = torch.eye(dim,dtype=A.dtype,device=A.devices)
...
def isqrt_newton_schulz_autograd_batch(A, numIters):
	...
	Y = A.div(normA)
	I = torch.eye(dim,dtype=A.dtype,device=A.devices).unsqueeze(0).expand_as(A)
	Z = torch.eye(dim,dtype=A.dtype,device=A.devices).unsqueeze(0).expand_as(A)
	...
	def forward(self, x):
		...
		if self.training and (not frozen):
			...
			if self.groups==1:
				...
				Id=torch.eye(X.shape[1], dtype=X.dtype, device=X.devices)
				Cov = torch.addmm(self.eps, Id, 1. / X.shape[0], X.t(), X)
				deconv = isqrt_newton_schulz_autograd(Cov, self.n_iter)
			else:
				X = X.view(-1, self.groups, self.num_features).transpose(0, 1)
				Id = torch.eye(self.num_features, dtype=X.dtype, device=X.devices).expand(self.groups, self.num_features, self.num_features)
