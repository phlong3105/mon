...
class Enhancement(object):
    ...
    def adjust_gammma(self, img_gray):
        ...
        image_gamma_correct = (image_gamma_correct.astype(np.float32)) / 255
    ...
    def get_enhanced(self, step, flag=False):
        ...
        # ini_illumination = t.resize(ini_illumination, (self.size[1], self.size[0]))
...
if __name__ == "__main__":
    ...
    input_root  = mon.DATA_DIR / "llie/test"
    output_root = mon.DATA_DIR / "llie/predict"
    
    datasets = ["dicm", "fusion", "lime", "lol", "npe", "vv", "ve-lol", "ve-lol-syn"]
    # datasets = ['images-for-computing-time']
    for dataset in datasets:
        input_folder  = input_root / dataset / "low"
        output_folder = output_root / "retinexdip" / dataset
        # input_folder  = os.path.join(input_root, dataset)
        # output_folder = os.path.join(output_root, dataset)
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
        # print(output_folder)
        path = list(input_folder.rglob("*"))
        path.sort()
        for i in range(len(path)):
            filename     = os.path.basename(str(path[i]))
