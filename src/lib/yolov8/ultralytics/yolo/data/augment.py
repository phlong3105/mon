...
from .utils import (
    IMAGENET_MEAN, IMAGENET_STD, polygons2masks,
...
# TODO: we might need a BaseTransform to make all these augments be
#  compatible with both classification and semantic
class BaseTransform:
...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, imgsz=640, p=1.0, border=(0, 0)):
        assert 0 <= p <= 1.0, "The probability should be in range [0, " \
                              "1]. " f"got {p}."
    ...
    def _mix_transform(self, labels):
        ...
        assert labels.get(
            "rect_shape",
            None
        ) is None, "rect and mosaic is exclusive."
        assert len(
            labels.get("mix_labels", [])
        ) > 0, "There are no other images for mosaic augment."
        s = self.imgsz
        yc, xc = (int(random.uniform(-x, 2 * s + x)) for x in
                  self.border)  # mosaic center x, y
        for i in range(4):
            labels_patch = (
                labels if i == 0 else labels["mix_labels"][i - 1]).copy()
            # Load image
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        labels["instances"] = Instances.concatenate(
            [labels["instances"], labels2["instances"]],
            axis=0
        )
...
class RandomPerspective:
    ...
    def affine_transform(self, img, border):
        ...
        T[0, 2] = random.uniform(0.5 - self.translate, 0.5 + self.translate) * \
                  self.size[0]  # x translation (pixels)
        T[1, 2] = random.uniform(0.5 - self.translate, 0.5 + self.translate) * \
                  self.size[1]  # y translation (pixels)
    ...
    def apply_bboxes(self, bboxes, M):
        ...
        Args:
            bboxes(ndarray): list of bboxes, xyxy format, with shape (
            num_bboxes, 4).
        ...
        xy = xy @ M.T  # transform
        xy = (
            xy[:, :2] / xy[:, 2:3] if self.perspective else xy[:, :2]).reshape(
            n,
            8
        ...
        x = xy[:, [0, 2, 4, 6]]
        y = xy[:, [1, 3, 5, 7]]
        return np.concatenate((x.min(1), y.min(1), x.max(1), y.max(1))).reshape(
            4,
            n
        ).T
    ...
    def apply_segments(self, segments, M):
        ...
        Args:
            ...
            M(ndarray): affine matrix.
        Returns:
            new_segments(ndarray): list of segments after affine,
            [num_samples, 500, 2].
        ...
        bboxes = np.stack(
            [segment2box(xy, self.size[0], self.size[1]) for xy in segments],
            0
        )
    ...
    def apply_keypoints(self, keypoints, M):
        ...
        x_kpts[np.logical_or.reduce(
            (x_kpts < 0, x_kpts > self.size[0], y_kpts < 0,
             y_kpts > self.size[1])
        )] = 0
    ...
    def __call__(self, labels):
        ...
        self.size = img.shape[1] + border[1] * 2, img.shape[0] + border[
            0] * 2  # w, h
        # M is affine matrix
    ...
    def box_candidates(
        ...
        # Compute box candidates: bbox1 before augment, bbox2 after augment,
...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            im_hsv = cv2.merge(
                (cv2.LUT(hue, lut_hue), cv2.LUT(sat, lut_sat),
                 cv2.LUT(val, lut_val))
            )
...
class RandomFlip:
    ...
    def __init__(self, p=0.5, direction="horizontal") -> None:
        assert direction in ["horizontal",
                             "vertical"], f"Support direction `horizontal` or " \
                                          f"`vertical`, got {direction}"
        assert 0 <= p <= 1.0
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if not self.scaleup:  # only scale down, do not scale up (for better
            # val mAP)
            r = min(r, 1.0)
        ...
        if labels.get("ratio_pad"):
            labels["ratio_pad"] = (
            labels["ratio_pad"], (dw, dh))  # for evaluation
...
# TODO: technically this is not an augmentation, maybe we should put this to
#  another files
class Format:
    ...
    def __call__(self, labels):
        ...
        labels["bboxes"] = torch.from_numpy(
            instances.bboxes
        ) if nl else torch.zeros((nl, 4))
        if self.return_keypoint:
            labels["keypoints"] = torch.from_numpy(
                instances.keypoints
            ) if nl else torch.zeros((nl, 17, 2))
...
def classify_transforms(size=224):
    ...
    assert isinstance(
        size,
        int
    ), f"ERROR: classify_transforms size {size} must be integer, not (list, " \
       f"tuple)"
    # T.Compose([T.ToTensor(), T.Resize(size), T.CenterCrop(size),
...
def classify_albumentations(
    ...
    try:
        ...
        T += [A.Normalize(mean=mean, std=std),
              ToTensorV2()]  # Normalize and convert to Tensor
        LOGGER.info(
            prefix + ", ".join(
                f"{x}".replace("always_apply=False, ", "") for x in T if x.p
            )
...
class ClassifyLetterBox:
    ...
    def __call__(self, im):  # im = np.array HWC
        ...
        hs, ws = (math.ceil(x / self.stride) * self.stride for x in
                  (h, w)) if self.auto else self.h, self.w
