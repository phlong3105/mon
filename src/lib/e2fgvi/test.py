...
def read_mask_from_videos(args, size):
    vname = args.mask
    frames = []
    if args.use_mp4:
...
def main_worker():
    ...
    args.use_mask_mp4 = True if args.mask.endswith('.mp4') else False
    if args.use_mask_mp4:
        masks = read_mask_from_videos(args, size)
    else:
    ...
    """
    save_path = args.output or os.path.join(save_dir_name, save_name)
    save_path = mon.Path(save_path)
    save_path.parent.mkdir(parents=True, exist_ok=True)
    writer    = cv2.VideoWriter(str(save_path), cv2.VideoWriter_fourcc(*"mp4v"), default_fps, size)
