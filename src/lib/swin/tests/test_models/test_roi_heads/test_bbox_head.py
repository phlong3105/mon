...
def test_bbox_head_loss():
    """Tests box head loss when truth is empty and non-empty."""
    ...
    # Test box loss when truth is empty
    ...
    # Create dummy features "extracted" for each sampled box
    ...
    losses = self.loss(
        cls_scores, bbox_preds, rois, labels, label_weights,
    ...
    # Test box loss when truth is non-empty
    gt_bboxes = [
...
def _demodata_refine_boxes(n_roi, n_img, rng=0):
    ...
    import numpy as np
    from mmdet.core.box.demodata import random_boxes
    from mmdet.core.box.demodata import ensure_rng
    try:
        import kwarray
    except ImportError:
        import pytest
