...
def _check_roi_head(config, head):
    ...
    if head.with_mask:
        ...
        if config.mask_roi_extractor:
            ...
            _check_roi_extractor(
                mask_roi_cfg, mask_roi_extractor,
                bbox_roi_extractor
            )
    ...
    if config['type'] in ['CascadeRoIHead', 'HybridTaskCascadeRoIHead']:
        ...
        _check_roi_extractor(
            grid_roi_cfg, grid_roi_extractor,
            bbox_roi_extractor
        )
    ...
    if isinstance(bbox_cfg, list):
        ...
        if bbox_cfg['type'] == 'SABLHead':
            ...
            assert bbox_cfg[
                   ...
                   bbox_head.ffn.feedforward_channels
...
def test_config_data_pipeline(config_rpath):
    ...
    mode = 'bitmap' if loading_ann_pipeline.get(
        'poly2mask',
        True
    ) else 'polygon'
    results = dict(
