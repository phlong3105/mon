...
def isr_p(
    ...
    Args:
        ...
        bbox_pred (Tensor): Predicted box deltas.
        bbox_targets (tuple[Tensor]): A tuple of box targets, the are
    ...
    # compute iou of the predicted box and the corresponding GT
...
def carl_loss(
    ...
    Args:
        ...
        bbox_pred (Tensor): Predicted box deltas.
        bbox_targets (Tensor): Target of box regression.
    ...
    # multiply pos_cls_score with the corresponding box weight
    # and remain gradient
    if sigmoid:
    ...
    # if is class agnostic, box pred is in shape (N, 4)
    # otherwise, box pred is in shape (N, #classes, 4)
    if bbox_pred.size(-1) > 4:
        bbox_pred = bbox_pred.view(bbox_pred.size(0), -1, 4)
