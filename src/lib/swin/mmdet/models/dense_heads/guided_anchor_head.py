...
from mmdet.core import (
    anchor_inside_flags, build_anchor_generator,
...
class GuidedAnchorHead(AnchorHead):
    ...
    Args:
        ...
        bbox_coder (dict): Config dict for box coder
        reg_decoded_bbox (bool): If true, the regression loss would be
            applied directly on decoded bounding boxes, converting both
            the predicted boxes and regression targets to absolute
            coordinates format. Default False. It should be `True` when
            using `IoULoss`, `GIoULoss`, or `DIoULoss` in the box head.
        ...
        loss_bbox (dict): Config of box regression loss.
    """
    ...
    def ga_loc_targets(self, gt_bboxes_list, featmap_sizes):
        ...
        for lvl_id in range(num_lvls):
            ...
            loc_targets = torch.zeros(
                ...
                h,
                w,
                device=gt_bboxes_list[0].devices,
                dtype=torch.float32
            )
    ...
    def loss_shape_single(
        self, shape_pred, bbox_anchors, bbox_gts,
        ...
        inds = torch.nonzero(
            anchor_weights[:, 0] > 0, as_tuple=False
        ).squeeze(1)
    ...
    def loss_loc_single(
        self, loc_pred, loc_target, loc_weight,
        loc_avg_factor
    ):
    ...
    def loss(
        ...
        # get classification and box regression losses
