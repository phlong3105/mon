...
from mmdet.core import (
    bbox2roi, bbox_mapping, merge_aug_bboxes,
...
class BBoxTestMixin(object):
    ...
    def simple_test_bboxes(
        ...
        batch_index = torch.arange(
            rois.size(0), device=rois.device
        ...
        if bbox_pred is not None:
            # the box prediction of some detectors like SABL is not Tensor
            if isinstance(bbox_pred, torch.Tensor):
                ...
                # apply box post-processing to each image individually
                det_bboxes = []
                det_labels = []
                for i in range(len(proposals)):
                    # remove padding
                    supplement_mask = proposals[i][..., -1] == 0
                    for box in bbox_preds[i]:
                        box[supplement_mask] = 0
    ...
    def simple_test_mask(
        ...
        if isinstance(det_bboxes, list):
            ...
            for i, (box, label) in enumerate(zip(det_bboxes, det_labels)):
                supplement_bbox = box.new_full(
                    (max_size - box.size(0), box.size(1)), 0
                )
                supplement_label = label.new_full(
                    (max_size - label.size(0),),
                    0
                )
                det_bboxes[i] = torch.cat((supplement_bbox, box), dim=0)
        ...
        batch_index = torch.arange(
            det_bboxes.size(0), device=det_bboxes.devices
        ).float().view(
            -1, 1, 1
