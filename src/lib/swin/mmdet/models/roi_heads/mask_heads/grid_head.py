...
class GridHead(nn.Module):
    ...
    def get_targets(self, sampling_results, rcnn_train_cfg):
        ...
        for i in range(num_rois):
            ...
            for j in range(self.grid_points):
                ...
                cx = int(
                    (gridpoint_x - pos_bboxes[i, 0]) / pos_bbox_ws[i] *
                    map_size
                    )
                cy = int(
                    (gridpoint_y - pos_bboxes[i, 1]) / pos_bbox_hs[i] *
                    map_size
                    )
        ...
        sub_targets = sub_targets.to(sampling_results[0].pos_bboxes.devices)
        return sub_targets
    ...
    def get_bboxes(self, det_bboxes, grid_pred, img_metas):
        ...
        # get the grid points indices that fall on the box boundaries
