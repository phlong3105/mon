...
from mmdet.core import (
    bbox2result, bbox2roi, bbox_mapping, merge_aug_bboxes,
...
class HybridTaskCascadeRoIHead(CascadeRoIHead):
    """Hybrid task cascade roi head including one box head and one mask head.
    ...
    def _bbox_forward(self, stage, x, rois, semantic_feat=None):
        ...
        if self.with_semantic and 'box' in self.semantic_fusion:
        ...
        if self.with_mask:
            if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
                ...
                if rescale and not isinstance(scale_factors[0], float):
                    scale_factors = [
                        torch.from_numpy(scale_factor).to(det_bboxes[0].devices)
