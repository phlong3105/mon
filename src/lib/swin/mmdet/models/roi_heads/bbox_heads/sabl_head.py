...
class SABLHead(nn.Module):
    ...
    Args:
        ...
        bbox_coder (dict): Config of box coder. Defaults 'BucketingBBoxCoder'.
        loss_cls (dict): Config of classification loss.
        loss_bbox_cls (dict): Config of classification loss for box branch.
        loss_bbox_reg (dict): Config of regression loss for box branch.
    """
    ...
    def _add_fc_branch(
        self, num_branch_fcs, in_channels, roi_feat_size,
        fc_out_channels
    ):
    ...
    def bbox_pred_split(self, bbox_pred, num_proposals_per_img):
        """Split batch box prediction back to each image."""
    ...
    def reg_forward(self, reg_x):
        ...
        offset_pred_x, cls_pred_x = self.reg_pred(
            reg_fx, self.reg_offset_fcs,
            self.reg_cls_fcs
        )
        offset_pred_y, cls_pred_y = self.reg_pred(
            reg_fy, self.reg_offset_fcs,
            self.reg_cls_fcs
        )
    ...
    def get_targets(
        self, sampling_results, gt_bboxes, gt_labels,
        rcnn_train_cfg
    ):
    ...
    def _bucket_target_single(
        self, pos_proposals, neg_proposals,
    ...
    def get_bboxes(
        ...
        if rescale and bboxes.size(0) > 0:
            if isinstance(scale_factor, float):
                ...
                bboxes /= torch.from_numpy(scale_factor).to(bboxes.devices)
    ...
    def refine_bboxes(self, rois, labels, bbox_preds, pos_is_gts, img_metas):
        ...
        Args:
            ...
            bbox_preds (list[Tensor]): Shape [(n*bs, num_buckets*2), \
                (n*bs, num_buckets*2)].
            pos_is_gts (list[Tensor]): Flags indicating if each positive box
                is a gt box.
    ...
    def regress_by_class(self, rois, label, bbox_pred, img_meta):
        """Regress the box for the predicted class. Used in Cascade R-CNN.
