...
from mmdet.models.roi_heads.mask_heads.condconv_mask_head import \
    aligned_bilinear
...
class RepPointsV2MaskDetector(SingleStageDetector):
    ...
    def __init__(
        ...
        super(RepPointsV2MaskDetector, self).__init__(
    ...
    def forward_train(
        ...
        Args:
            ...
            gt_masks (None | list[BitmapMasks]) : true segmentation masks for
            each box
        ...
        for mask in gt_masks:
            ...
            mask_tensor = F.pad(
                mask_tensor,
                pad=(0, img.size(-1) - mask_tensor.size(-1), 0,
                     img.size(-2) - mask_tensor.size(-2))
            )
    ...
    def mask2result(
        ...
        pred_instances.boxsz = torch.stack(
            (det_bboxes[:, 2] * scale_factor[2] - det_bboxes[:, 0] *
             scale_factor[0],
             det_bboxes[:, 3] * scale_factor[3] - det_bboxes[:, 1] *
             scale_factor[1]), axis=-1
        )
        mask_logits = self.bbox_head.mask_head(x, pred_instances)
        if len(pred_instances) > 0:
            mask_logits = aligned_bilinear(
                mask_logits,
                self.bbox_head.mask_head.head.mask_out_stride
            )
            ...
            mask_pred = torch.zeros(
                (self.bbox_head.num_classes, *img_meta['ori_shape'][:2]),
                dtype=torch.int
            )
        cls_segms = [[] for _ in range(
        ...
        for i, label in enumerate(det_labels):
            ...
            if self.mask_inbox:
                ...
                det_bbox_[[0, 1]], det_bbox_[[2, 3]] = det_bbox_[
                    [0, 1]].floor(), det_bbox_[[2, 3]].ceil()
                det_bbox_ = det_bbox_.int()
                mask_pred_[det_bbox_[1]:det_bbox_[3],
                det_bbox_[0]:det_bbox_[2]] = mask_pred[i][
                                             det_bbox_[1]:det_bbox_[3],
                                             det_bbox_[0]:det_bbox_[2]]
                cls_segms[label].append(mask_pred_.cpu().numpy())
            else:
                cls_segms[label].append(
                    mask_pred[i].cpu().numpy() if isinstance(
                        mask_pred[i],
                        torch.Tensor
                    ) else mask_pred[i]
                )
    ...
    def simple_test(self, img, img_metas, rescale=False):
        ...
        if not self.bbox_head.mask_head:  # detection only
            ...
            cls_segms = [
                ...
                for i, (det_bboxes, det_labels, inst_inds) in
                enumerate(bbox_list)
            ]
