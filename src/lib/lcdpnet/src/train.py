...
def main(config):
    ...
    callbacks = []
    if opt[EARLY_STOP]:
        print(f"Apply EarlyStopping when `{opt.checkpoint_monitor}` is {opt.monitor_mode}")
        callbacks.append(EarlyStopping(opt.checkpoint_monitor, mode=opt.monitor_mode))
    ...
    if opt[AMP_BACKEND] != "native":
        print(f"WARN: Running in APEX, mode: {opt[AMP_BACKEND]}-{opt[AMP_LEVEL]}")
    else:
        opt[AMP_LEVEL] = None
