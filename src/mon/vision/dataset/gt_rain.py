...
@constant.DATASET.register(name="gt-rain")
class GTRain(base.ImageEnhancementDataset):
    ...
    def __init__(
        self,
        name            : str                    = "gt-rain",
        root            : PathType               = constant.DATA_DIR / "gt-rain",
        split           : str                    = "train",
    ...
    def list_labels(self):
        ...
        with core.rich.progress_bar() as pbar:
            for img in pbar.track(
                ...
                path = str(img.path)
                path = core.Path(path[: path.find("-R-")] + "-C-000.png")
                self.labels.append(
                    base.ImageLabel(path=path, backend=self.backend)
                )
...
@constant.DATAMODULE.register(name="gt-rain")
...
def test_gt_rain():
    cfg = {
        "name": "gt-rain",
            # A datamodule's name.
        "root": constant.DATA_DIR / "gt-rain",
...
def parse_args():
    ...
    parser.add_argument("--task", type=str , default="test-gt-rain", help="The task to run")
