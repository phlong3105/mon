...
from utils.general import (
    ap_per_class, box_iou, check_file, check_img_size, clip_coords,
    coco80_to_coco91_class, compute_loss, non_max_suppression, output_to_target,
    plot_images, scale_coords, xywh2xyxy, xyxy2xywh,
)
...
def test(
    ...
    if training:  # called by train.py
        device = next(model.parameters()).devices  # get model device
    
    else:  # called directly
        device = select_device(opt.device, batch_size=batch_size)
        merge, save_txt = opt.merge, opt.save_txt  # use Merge NMS,
        # save *.txt labels
        if save_txt:
    ...
    if not training:
        ...
        dataloader = create_dataloader(
            path, imgsz, batch_size, model.stride.max(), opt,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(
        ...
        with torch.no_grad():
            ...
            # inf_out, train_out = model(img.to(
            # memory_format=torch.channels_last), augment=augment)  #
            # inference and training outputs
            ...
            if training:  # if model has loss hyperparameters
                loss += \
                compute_loss([x.float() for x in train_out], targets, model)[1][
                :3]  # GIoU, obj, cls
        ...
        for si, pred in enumerate(output):
            ...
            if save_json:
                # [{"image_id": 42, "category_id": 18, "box": [258.15, 41.29,
    ...
    if not training:
        print(
            'Speed: %.1f/%.1f/%.1f ms inference/NMS/total per %gx%g image at '
            'batch-size %g' % t
            )
    ...
    if save_json and len(jdict):
        f = 'detections_val2017_%s_results.json' % \
            (weights.split(os.sep)[-1].replace('.pt', '') if isinstance(
                weights,
                str
            ) else '')  # filename
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI
            ...
            cocoEval = COCOeval(cocoGt, cocoDt, 'box')
...
if __name__ == '__main__':
    ...
    if opt.task in ['val', 'test']:  # run normally
        ...
        for weights in ['']:
            f = 'study_%s_%s.txt' % (
            Path(opt.data).stem, Path(weights).stem)  # filename to save to
