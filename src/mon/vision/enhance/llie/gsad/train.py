...
def train(args: argparse.Namespace):
    ...
    weights  = args.weights[0] if isinstance(args.weights, (list, tuple)) else args.weights
    device   = args.device
    ...
    opt_dataset = option.parse(args.data, is_train=True)
    ...
    if "uncertainty" in str(args.config):
...
@click.option("--config",     type=str, default="",   help="Model config.")
@click.option("--model",      type=str, default=None, help="*.cfg path.")
@click.option("--data",       type=str, default=None, help="data.yaml path.")
...
@click.option("--weights",    type=str, default=None, help="Initial weights path.")
@click.option("--device",               default=None, help="Running devices.")
...
def main(
    ...
    config_file = config_file if config_file.exists() else _current_dir / "config/train" / core.Path(config_file).name
    config_file = core.Path(config_file).config_file()
    if not config_file.exists():
        raise FileNotFoundError(f"Config file not found: {config_file}")
    
    # Prioritize input args --> config file args
    data     = core.Path(data)
    data     = data if data.exists() else _current_dir / "config/data" / data.name
