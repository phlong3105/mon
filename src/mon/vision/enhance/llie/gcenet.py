...
class DnCNN(nn.Module):
    ...
    def __init__(
        ...
        layers      = []
        layers.append(nn.Conv2d(self.channels, self.num_channels, kernel_size, padding=padding, bias=False))
        ...
        for _ in range(self.num_layers - 2):
            ...
            layers.append(nn.ReLU(inplace=True))
        layers.append(nn.Conv2d(self.num_channels, self.channels, kernel_size, padding=padding, bias=False))
...
class GCENet(base.LowLightImageEnhancementModel):
    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        self.enc1     = nn.DSConv2d(self.channels,         self.num_channels, 3, 1, 1, bias=True)
        ...
        self.dec1     = nn.DSConv2d(self.num_channels * 2, 3,     3, 1, 1, bias=True)
        self.act      = nn.PReLU()
...
@MODELS.register(name="gcenet3+")
class GCENet3P(base.LowLightImageEnhancementModel):
    """Guidance Curve Estimation (GCENet) model.
    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        # Construct model
        self.enc1 = nn.DSConv2d(self.channels + 1,         self.num_channels, 3, 1, 1, bias=True)
        self.enc2 = nn.DSConv2d(self.num_channels + 1,     self.num_channels, 3, 1, 1, bias=True)
        self.enc3 = nn.DSConv2d(self.num_channels + 1,     self.num_channels, 3, 1, 1, bias=True)
        self.mid  = nn.DSConv2d(self.num_channels + 1,     self.num_channels, 3, 1, 1, bias=True)
        self.dec3 = nn.DSConv2d(self.num_channels * 2 + 1, self.num_channels, 3, 1, 1, bias=True)
        self.dec2 = nn.DSConv2d(self.num_channels * 2 + 1, self.num_channels, 3, 1, 1, bias=True)
        self.dec1 = nn.DSConv2d(self.num_channels * 2 + 1, 3,     3, 1, 1, bias=True)
        self.act  = nn.PReLU()
    ...
    def forward(
        ...
        *args, **kwargs
        ...
        g = proc.get_guided_brightness_enhancement_map_prior(x, self.gamma, 9)
        #
        enc1      = self.act(self.enc1(torch.cat([x, g], dim=1)))
        enc2      = self.act(self.enc2(torch.cat([enc1, g], dim=1)))
        enc3      = self.act(self.enc3(torch.cat([enc2, g], dim=1)))
        mid       = self.act( self.mid(torch.cat([enc3, g], dim=1)))
        dec3      = self.act(self.dec3(torch.cat([mid,  enc3, g], dim=1)))
        dec2      = self.act(self.dec2(torch.cat([dec3, enc2, g], dim=1)))
        dec1      =   F.tanh(self.dec1(torch.cat([dec2, enc1, g], dim=1)))
        l         = dec1
        # Enhancement
        if not self.predicting:
            y = x
