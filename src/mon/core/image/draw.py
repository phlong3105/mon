...
def draw_heatmap(
    ...
    if np.max(image) > 1:
        raise ValueError(
            f"`image` should be an `np.float32` in the range ```[0.0, 1.0]```, "
            f"but got {np.max(image)}."
        )
    if not 0 <= alpha <= 1:
        raise ValueError(
            f"`alpha` should be in the range ```[0.0, 1.0]```, but got: {alpha}."
        )
...
def draw_trajectory(
    ...
    if isinstance(trajectory, list):
        if not all(len(t) == 2 for t in trajectory):
            raise TypeError(
                f"`trajectory` must be a list of points in `[(x1, y1), ...]` "
                f"format."
            )
