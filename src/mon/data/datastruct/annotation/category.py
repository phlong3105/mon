...
def logits_to_class_id(logits: np.ndarray) -> np.ndarray:
    ...
    Args:
        logits: A :class:`numpy.ndarray` of logits where each row corresponds
            to a set of logits for a sample.

    Returns:
        A :class:`numpy.ndarray` of class IDs corresponding to the highest logit
...
def class_id_to_logits(
    ...
    Returns:
        A :class:`numpy.ndarray` represents the logits for the given class ID.
    """
...
class ClassificationAnnotation(base.Annotation):
    ...
    Args:
        ...
        num_classes: The total number of classes in the classification task.
        confidence: A confidence in :math:`[0, 1]` for the classification.
            Default: ``1.0``.
    ...
    def collate_fn(batch: list[torch.Tensor | np.ndarray]) -> torch.Tensor | np.ndarray | None:
        ...
        Args:
            batch: A :class:`list` of class ids.
        """
