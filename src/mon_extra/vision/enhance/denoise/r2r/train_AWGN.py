...
from dataset import Dataset_train, Dataset_val, prepare_data
...
def main():
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            if opt.training == 'R2R':
                eps          = sigma / 255.0
                pert         = eps * torch.FloatTensor(img_train.size()).normal_(mean=0, std=1.0).cuda()
            ...
            out_train  = torch.clamp(model(img_train), 0.0, 1.0)
            psnr_train = batch_PSNR(out_train, clean, 1.0)
            print("%s [epoch %d][%d/%d] loss: %.4f  PSNR: %.4f" % (opt.training, epoch + 1, i + 1, len(loader_train), loss.item(), psnr_train))
        ...
        if opt.training == 'R2R':
            if (epoch + 1) % 1 == 0 or epoch == 0:
                ...
                for k in range(len(dataset_val)):
                    ...
                    out_val        = torch.clamp(out_val/aver_num, 0.0, 1.0)
                    psnr_val_pert += batch_PSNR(out_val, img_val, 1.0)
                    
                    with torch.no_grad():
                        out_val = torch.clamp(model(imgn_val), 0.0, 1.0)
