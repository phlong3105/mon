...
def train(args: argparse.Namespace):
    ...
    val_dataset = lowlight_loader(
        images_path = str(args.input_val),
        mode        = "test",
        normalize   = args.normalize
    )
    ...
    with mon.get_progress_bar() as pbar:
        for epoch in pbar.track(
            ...
            for iteration, images in enumerate(train_loader):
                ...
                loss = L1_smooth_loss(enhance_img, high_image) + 0.04 * loss_network(enhance_img, high_image)
                loss.backward()
                optimizer.step()
                scheduler.step()
