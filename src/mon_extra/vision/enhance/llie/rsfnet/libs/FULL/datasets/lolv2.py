...
class lolv2(Dataset):
    def __init__(self, config, mode):
        ...
        if mode=='train':
            ...
            if (config.f_valFromTrain==True) and config.p_valList==None:       # USE 10% of training as validation
                with open(config.p_trainList) as f:
                    self.imList     = [l.strip() for l in f.readlines()]
            ...
            # self.augment     = transforms.Compose([transforms.Resize(size=(config.imsize,config.imsize)), transforms.RandomHorizontalFlip(), transforms.ToTensor()])
            # self.augment     = transforms.Compose([transforms.RandomHorizontalFlip(), transforms.ToTensor()])
            ...
            # self.augment     = transforms.Compose([transforms.Resize(size=(config.imsize,config.imsize)),transforms.ToTensor()]) #  RESIZE OR NOT ?
            self.augment     = transforms.Compose([transforms.ToTensor()])
    ...
    def __getitem__(self,idx):
        imName  = self.imList[idx].split('.')[0]
        imNum   = imName.split('low')[1]
        p_low   = os.path.join(self.p_inDir, 'low'+imNum+'.png')
        imlow   = Image.open(p_low)
        if not self.f_RGB:  imlow  = imlow.convert('YCbCr')
        imlow   = self.augment(imlow)
        p_gt    = os.path.join(self.p_gtDir, 'normal'+imNum+'.png')
