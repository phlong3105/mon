...
from one.vision.detection.scaled_yolov4.utils.general import ap_per_class
from one.vision.detection.scaled_yolov4.utils.general import box_iou
...
from one.vision.detection.scaled_yolov4.utils.general import clip_coords
...
from one.vision.detection.scaled_yolov4.utils.torch_utils import \
    time_synchronized
...
def parse_args():
    ...
    parser.add_argument("--weights"   , default=weights_dir/"yolov4_p7_coco.pt", type=str, nargs="+", help="model.pt path(s)")
    ...
    if args.task in ["val", "test"]:  # run normally
        ...
        for weights in [""]:
            ...
            for i in x:  # img-size
                ...
                r, _, t = test(args.data, weights, args.batch_size, i, args.conf_thres, args.iou_thres, args.save_json)
