...
from one.constants import PRETRAINED_DIR, DATA_DIR
from one.vision.detection.yolov5_v6_1.models.common import DetectMultiBackend
...
from one.vision.detection.yolov5_v6_1.utils.general import box_iou
...
from one.vision.detection.yolov5_v6_1.utils.general import coco80_to_coco91_class
...
from one.vision.detection.yolov5_v6_1.utils.general import non_max_suppression
from one.vision.detection.yolov5_v6_1.utils.general import print_args
from one.vision.detection.yolov5_v6_1.utils.general import scale_coords
from one.vision.detection.yolov5_v6_1.utils.general import xywh2xyxy
from one.vision.detection.yolov5_v6_1.utils.general import xyxy2xywh
from one.vision.detection.yolov5_v6_1.utils.metrics import ap_per_class
from one.vision.detection.yolov5_v6_1.utils.metrics import ConfusionMatrix
from one.vision.detection.yolov5_v6_1.utils.plots import output_to_target
from one.vision.detection.yolov5_v6_1.utils.plots import plot_images
from one.vision.detection.yolov5_v6_1.utils.plots import plot_val_study
...
def parse_args():
    ...
    parser.add_argument("--weights",     default=weights_dir/"yolov5x6_v6_1_coco.pt", nargs="+", type=str, help="model.pt path(s)")
...
def main(args):
    ...
    if args.task in ("train", "val", "test"):  # run normally
        if args.conf_thres > 0.001:  # https://github.com/ultralytics/yolov5/issues/1466
            LOGGER.info(f"WARNING: confidence threshold {args.conf_thres} >> 0.001 will produce invalid mAP values.")
        run(**vars(args))

    else:
        weights = args.weights if isinstance(args.weights, list) else [args.weights]
        args.half = True  # FP16 for fastest results
        if args.task == "speed":  # speed benchmarks
            ...
            for args.weights in weights:
                f = f"study_{Path(args.data).stem}_{Path(args.weights).stem}.txt"  # filename to save to
