...
from one.vision.detection.yolov7.utils.general import check_requirements
from one.vision.detection.yolov7.utils.general import clean_str
from one.vision.detection.yolov7.utils.general import resample_segments
from one.vision.detection.yolov7.utils.general import segment2box
from one.vision.detection.yolov7.utils.general import segments2boxes
from one.vision.detection.yolov7.utils.general import xyn2xy
from one.vision.detection.yolov7.utils.general import xywh2xyxy
from one.vision.detection.yolov7.utils.general import xywhn2xyxy
from one.vision.detection.yolov7.utils.general import xyxy2xywh
from one.vision.detection.yolov7.utils.torch_utils import torch_distributed_zero_first
...
def autosplit(
    path          : Union[str, Path] = "../coco",
    weights       : Sequence[float]  = (0.9, 0.1, 0.0),
...
def copy_paste(img, labels, segments, probability: float = 0.5):
...
def exif_size(img: Image) -> tuple[int, int]:
...
def flatten_recursive(path: Union[str, Path] = "../coco"):
...
def get_hash(files: list) -> int:
    # Returns a single hash value of a list of files
    return sum(os.path.getsize(f) for f in files if os.path.isfile(f))


def hist_equalize(img, clahe: bool = True, bgr: bool = False):
...
def img2label_paths(img_paths: Union[list, tuple]) -> list:
...
def load_mosaic9(self, index: int):
...
def load_samples(self, index: int):
    """Loads images in a 4-mosaic."""
    labels4   = []
    segments4 = []
    s         = self.img_size
...
def load_segmentations(self, index: int):
...
def sample_segments(
    img, labels, segments, probability: float = 0.5
...
class LoadWebcam:  # for inference
    ...
    def __init__(
        self, pipe: Union[int, str] = "0", img_size: int = 640, stride: int = 32
    ):
        self.img_size = img_size
        self.stride   = stride
...
class Albumentations:
    ...
    def __call__(self, im, labels, p: float = 1.0):
...
def create_dataloader(
    ...
    quad         : bool  = False,
    prefix       : str   = ""
) -> tuple[Union[DataLoader, InfiniteDataLoader], LoadImagesAndLabels]:
