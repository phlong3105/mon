...
from one.vision.detection.yolov7.utils.general import bbox_iou
from one.vision.detection.yolov7.utils.general import box_iou
from one.vision.detection.yolov7.utils.general import xywh2xyxy
from one.vision.detection.yolov7.utils.torch_utils import is_parallel
...
class SigmoidBin(nn.Module):
    ...
    def __init__(
        ...
        max                : float = 1.0,
        reg_scale          : float = 2.0,
        use_loss_regression: bool  = True,
        use_fw_regression  : bool  = True,
    ...
    def training_loss(self, pred: Tensor, target: Tensor) -> tuple[Tensor, Tensor]:
        assert pred.shape[-1] == self.length,     "pred.shape[-1]=%d is not equal to self.length=%d"  % (pred.shape[-1], self.length)
...
class RankSort(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, logits, targets, delta_RS: float = 0.50, eps: float = 1e-10):
        ...
        for ii in order:
            ...
            mis_sorted_examples = (~ iou_relations) * fg_relations

            # Denominator of sorting pmf
            sorting_pmf_denom = torch.sum(mis_sorted_examples)
            ...
            if sorting_pmf_denom > eps:
                ...
                # For positives, distribute error via sorting pmf (i.e. mis_sorted_examples/sorting_pmf_denom)
                fg_grad     += (mis_sorted_examples*(sorting_error[ii]/sorting_pmf_denom))
...
class aLRPLoss(torch.autograd.Function):
    @staticmethod
    def forward(ctx, logits, targets, regression_losses, delta: float = 1.0, eps: float = 1e-5):
        classification_grads = torch.zeros(logits.shape).cuda()
...
class APLoss(torch.autograd.Function):
    @staticmethod
    def forward(ctx, logits, targets, delta: float = 1.0):
        classification_grads = torch.zeros(logits.shape).cuda()
...
class ComputeLossOTA:
    ...
    def __init__(self, model, auto_balance: bool = False):
        ...
        det               = model.module.model[-1] if is_parallel(model) else model.model[-1]  # Detect() module
        self.balance      = {3: [4.0, 1.0, 0.4]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7
        self.ssi          = list(det.stride).index(16) if auto_balance else 0  # stride 16 index
        self.BCEcls       = BCEcls
        self.BCEobj       = BCEobj
        self.gr           = model.gr
        self.hyp          = h
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                # pxy = (fg_pred[:, :2].sigmoid() * 3.0 - 1.0 + grid) * self.stride[i]
