...
from one import DropPath
from one import ListOrTupleAnyT
...
def autopad(k, p=None) -> Union[int, list[int]]:  # kernel, padding
	# Pad to 'same'
	if p is None:
...
class SP(nn.Module):
	...
	def __init__(self, k: int = 3, s: int = 1):
...
class ReOrg(nn.Module):
	...
	def forward(self, x: Tensor) -> Tensor:  # x(b,c,w,h) -> y(b,4c,w/2,h/2)
...
def DWConv(c1: int, c2: int, k: int = 1, s: int = 1, act: bool = True):
...
class DownC(nn.Module):
	...
	def __init__(self, c1: int, c2: int, n: int = 1, k: int = 2):
...
class SPP(nn.Module):
	...
	def __init__(self, c1: int, c2: int, k: Union[list, tuple] = (5, 9, 13)):
...
class Ghost(nn.Module):
	...
	def __init__(self, c1: int, c2: int, k: int = 3, s: int = 1):  # ch_in, ch_out, kernel, stride
...
class Detections:
	...
	def save(self, save_dir: str = "runs/hub/exp"):
...
class WindowAttention_v2(nn.Module):
	...
	def __init__(
		...
		attn_drop             : float = 0.0,
		proj_drop             : float = 0.0,
		pretrained_window_size: ListOrTupleAnyT[int] = (0, 0)
	):
