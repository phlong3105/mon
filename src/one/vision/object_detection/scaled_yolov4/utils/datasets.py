...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        # img0 = cv2.rotate(img0, cv2.ROTATE_180)  # For aic22retail dataset
        # Padded resize
...
class LoadBatchImages:  # for inference
    
    def __init__(self, path, img_size=640, batch_size=1):
        p = str(Path(path))  # os-agnostic
        p = os.path.abspath(p)  # absolute path
        if "*" in p:
            files = sorted(glob.glob(p))  # glob
    ...
    def __next__(self):
        ...
        # cv2.imwrite(path + ".letterbox.jpg", 255 * img.transpose((1, 2, 0))[:, :, ::-1])  # save letterbox image
        # return path, img, img0, self.cap
        return paths, np.array(images), np.array(images0), self.cap
