...
from one.vision.object_detection.yolov5_v6_1.utils.downloads import gsutil_getsize
from one.vision.object_detection.yolov5_v6_1.utils.metrics import box_iou
from one.vision.object_detection.yolov5_v6_1.utils.metrics import fitness

# Settings
FILE        = Path(__file__).resolve()
ROOT        = FILE.parents[1]  # YOLOv5 root directory
DATA_DIR    = ROOT.parent / "data"  # YOLOv5 datasets directory
...
def check_dataset(data, autodownload=True):
    ...
    if isinstance(data, (str, Path)) and str(data).endswith(".zip"):  # i.e. gs://bucket/dir/coco128.zip
        download(data, dir=DATA_DIR, unzip=True, delete=False, curl=False, threads=1)
        data = next((DATA_DIR / Path(data).stem).rglob("*.yaml"))
