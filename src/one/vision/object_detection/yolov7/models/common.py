...
from one.vision.object_detection.yolov7.utils.datasets import letterbox
from one.vision.object_detection.yolov7.utils.general import increment_path
from one.vision.object_detection.yolov7.utils.general import make_divisible
from one.vision.object_detection.yolov7.utils.general import non_max_suppression
from one.vision.object_detection.yolov7.utils.general import scale_coords
from one.vision.object_detection.yolov7.utils.general import xyxy2xywh
from one.vision.object_detection.yolov7.utils.plots import color_list
from one.vision.object_detection.yolov7.utils.plots import plot_one_box
from one.vision.object_detection.yolov7.utils.torch_utils import time_synchronized
...
class ConvBN(nn.Module):
	...
	def switch_to_deploy(self):
		...
		conv = nn.Conv2d(
			in_channels=self.conv.in_channels, out_channels=self.conv.out_channels, kernel_size=self.conv.kernel_size,
...
class RepConv_OREPA(nn.Module):
	...
	def __init__(self, c1, c2, k=3, s=1, padding=1, dilation=1, groups=1, padding_mode='zeros', deploy=False, use_se=False, nonlinear=nn.SiLU()):
		...
		if deploy:
			self.rbr_reparam = nn.Conv2d(
				in_channels=self.in_channels, out_channels=self.out_channels, kernel_size=k, stride=s,
	...
	def switch_to_deploy(self):
		...
		kernel, bias = self.get_equivalent_kernel_bias()
		self.rbr_reparam = nn.Conv2d(
			in_channels=self.rbr_dense.in_channels, out_channels=self.rbr_dense.out_channels,
...
class SwinTransformerLayer(nn.Module):
	...
	def __init__(
		self, dim, num_heads, window_size=8, shift_size=0,
