...
from one.vision.object_detection.yolov7.utils.general import check_requirements
from one.vision.object_detection.yolov7.utils.general import clean_str
from one.vision.object_detection.yolov7.utils.general import resample_segments
from one.vision.object_detection.yolov7.utils.general import segment2box
from one.vision.object_detection.yolov7.utils.general import segments2boxes
from one.vision.object_detection.yolov7.utils.general import xyn2xy
from one.vision.object_detection.yolov7.utils.general import xywh2xyxy
from one.vision.object_detection.yolov7.utils.general import xywhn2xyxy
from one.vision.object_detection.yolov7.utils.general import xyxy2xywh
from one.vision.object_detection.yolov7.utils.torch_utils import torch_distributed_zero_first
...
def get_hash(files: list):
    # Returns a single hash value of a list of files
    return sum(os.path.getsize(f) for f in files if os.path.isfile(f))


def exif_size(img: Image):
...
def sample_segments(img, labels, segments, probability: float = 0.5):
    # Implement Copy-Paste augmentation https://arxiv.org/abs/2012.07177, labels as nx5 np.array(cls, xyxy)
    n             = len(segments)
    sample_labels = []
    sample_images = []
