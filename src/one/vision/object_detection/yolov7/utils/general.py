...
from one.vision.object_detection.yolov7.utils.google_utils import gsutil_getsize
from one.vision.object_detection.yolov7.utils.metrics import fitness
from one.vision.object_detection.yolov7.utils.torch_utils import init_torch_seeds
...
def check_img_size(img_size, s: int = 32):
...
def labels_to_class_weights(labels, nc: int = 80):
...
def labels_to_image_weights(labels, nc: int = 80, class_weights=np.ones(80)):
...
def xyn2xy(x, w: int = 640, h: int = 640, padw: int = 0, padh: int = 0):
