...
label_pattern = os.path.join(
	data_dir, "uavdt", "mot", split, "annotations", "*_gt_whole.txt"
)
	...
	for file in pbar.track(
		...
		for k, v in line_dict.items():
			image_file = os.path.join(
				data_dir, "uavdt", "mot", split, "images", image_subdir, f"img{k}.jpg"
			)
			...
			label_file = os.path.join(
				data_dir, "uavdt", "mot", split, "labels", image_subdir, f"img{k}.txt"
			)
