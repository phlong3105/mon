...
class DeepUPE(ImageEnhancementDataset):
    ...
    def list_images(self):
        ...
        if self.split not in ["test"]:
            console.log(
                f"DeepUPE dataset only supports `split`: `test`. "
                f"Get: {self.split}."
            )
    ...
    def list_labels(self):
        ...
        with progress_bar() as pbar:
            for img in pbar.track(
                self.images,
                description=f"[bright_yellow]Listing DeepUPE {self.split} labels"
            ):
