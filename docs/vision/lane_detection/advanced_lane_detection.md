...
common knowledge that the lane lines are in fact parallel. Source and destination 
...
The third transformation is on the saturation component using the HLS colorspace. 
...
are fitted with a second degree polynomial function for each left and right side.
...
pixels that means that the car is shifted to the left by `nbp * xm_per_pix meters`. 
