...
def compute_gaussian_pyramid(tensor: torch.Tensor, n_levels: int = 4):
    b, c, _, _ = tensor.size()
    assert c in [1, 3]
    ...
    if c == 3:
        tensor = torch.cat([tensor[:, 0], tensor[:, 1], tensor[:, 2]], dim=0).unsqueeze(1)
...
def expand_stage(tensor: torch.Tensor, target_shape: tuple[int, int] = None):
    ...
    Args:
        ...
        target_shape: The target shape. The default is None, meaning that the
            target shape is ``(2h-1. 2w-1)``. If the target is bigger, the
            output interpolation will be padded.
    """
    _, _, h, w = tensor.size()
    H   = 2 * h - 1
    W   = 2 * w - 1
    out = F.interpolate(tensor, size=(H, W), mode="bilinear", align_corners=True)
...
def compute_laplacian_pyramid(gaussian_pyramid):
    ...
    for lvl in range(n_levels-2, -1, -1):
        ...
        expanded_g             = expand_stage(gaussian_pyramid[lvl + 1], target_shape=target_shape)
...
def merge_laplacian_pyramid(img_pyramid, weight_pyramid):
    ...
    for lvl, (weight, img) in enumerate(zip(weight_pyramid, img_pyramid)):
        merged_pyramid[lvl] = torch.sum(weight * img, dim=0, keepdim=True)
...
def collapse_pyramid(laplacian_pyramid):
    ...
    for stage in laplacian_pyramid[-2::-1]:  # Reverse order, starting from the penultimate
        ...
        curr = expand_stage(curr, target_shape=target_shape)
        curr = curr + stage
    return curr
