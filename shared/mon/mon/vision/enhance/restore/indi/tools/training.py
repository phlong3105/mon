...
from indi_diffusion import Indi_cond, psnr
# from utils import plot_images, save_images, get_data
from modules import EMA, IndiUnet
...
class PNGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        name  = os.path.join(self.root_dir, self.image_files[idx])
        image = Image.open(name).convert("RGB")  # Convert to RGB if needed
        if self.transform:
...
model          = IndiUnet(1, 1, imgsz=int(args.image_size), true_imgsz=args.image_size).to(device)
diffusion      = Indi_cond(imgsz=args.image_size)
