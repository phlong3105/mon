...
"""This module implements fisheye tomography transformation using Albumentations.
...
class FisheyeTomographyTransform(DualTransform):
    """Applies fisheye tomography transformation (FTT) to input image with
    accompanying mask and bounding boxes.

    Args:
        f: Focal length of the fisheye lens. Default: ``150``.
        imgsz: Output image size. Default: ``720``.
        bg_color: Background color for the fisheye image. Default: ``(0, 0, 0)``.
        bg_label: Background label for the fisheye image. Default: ``20``.
        reuse: Whether to reuse the transformation parameters for the next image.
            Default: ``False``.
    ...
    def __init__(
        ...
        self.ratio    = min(self.imgsz[0], self.imgsz[1]) / (self.f * math.pi)
    ...
    def random_ext_params(self):
        ...
        tmp1         = random.random()
        self.alpha   = self.alpha_range[0] * (1 - tmp1) + self.alpha_range[1] * tmp1
        tmp2         = random.random()
        self.beta    = self.beta_range[0] * (1 - tmp2) + self.beta_range[1] * tmp2
        tmp3         = random.random()
        self.theta   = self.theta_range[0] * (1 - tmp3) + self.theta_range[1] * tmp3
        tmp4         = random.random()
        self.x_trans = self.x_trans_range[0] * (1 - tmp4) + self.x_trans_range[1] * tmp4
        tmp5         = random.random()
    ...
    def _calculate_coord_map(self, image: np.ndarray):
        ...
        theta_array  = radius_array / self.f

        new_x_array  = np.tan(theta_array) * cord[:, 0] / radius_array * self.f
        new_y_array  = np.tan(theta_array) * cord[:, 1] / radius_array * self.f
    ...
    def _transform_image(self, image: np.ndarray) -> np.ndarray:
        ...
        fisheye[0, fisheye.shape[1] - 1] = self.bg_color
    ...
    def _transform_mask(self, image: np.ndarray) -> np.ndarray:
        ...
        fisheye[0, fisheye.shape[1] - 1] = self.bg_label
        fisheye = np.array(fisheye[(self.map_rows, self.map_cols)])
        fisheye = fisheye.reshape(self.imgsz[0], self.imgsz[1], 3)
        return fisheye
