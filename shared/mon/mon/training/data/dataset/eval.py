...
class ImageEvalDataset(Dataset):
    """A dataset class for the image quality assessment (IQA).
    
    The dataset includes two main modalities: ``image`` and ``target`` (optional).
    ...
    Attributes:
        ...
        target_dir (Path): Absolute path to the target data directory.
            ...
            with create_progress_bar(disable=self.disable_pbar) as pbar:
                ...
                for image in pbar.track(sequence=images, description=desc):
                    ...
                    if target_file.is_image_file():
                        targets.append(Image(path=target_file, root=self.target_dir))
        self._datapoints["target"] = targets
