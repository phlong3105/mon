...
class InitDataset(Dataset):
    ...
    def __init__(self, dataset_opt):
        ...
        opt      = dataset_opt
        root_dir = ROOT_DIR / "data" / str(opt["name"])
        
        filepath_EX  = get_file_paths(str(root_dir / "exposure"),       "png")
        filepath_BC  = get_file_paths(str(root_dir / "black_clipping"), "png")
        filepath_VB  = get_file_paths(str(root_dir / "vibrance"),       "png")
...
class FinetuneDataset(Dataset):
    ...
    def __init__(self, opt, phase):
        ...
        if self.opt["name"] == "fivek_dark":
            self.suffix_A   = "tif"
            self.suffix_B   = "jpg"
            self.dataroot_A = root_dir / "train/image"  # opt["dataroot"]
            self.dataroot_B = root_dir / "train/ref"    # opt["dataroot"]
            ...
            self.suffix_M   = "png"
            self.dataroot_A = opt["dataroot"]
            self.dataroot_B = os.path.join(opt["dataroot"], sopt["name"])
        else:
        ...
        if self.phase == "train":
            ...
            self.dataroot_A = root_dir / "test/image"
            self.dataroot_B = root_dir / "test/ref"
        self.path_A = get_file_paths(str(self.dataroot_A), "png")
        self.path_B = get_file_paths(str(self.dataroot_B), "png")
    ...
    def __getitem__(self, index):
        ...
        if pathlib.Path(path_A).suffix == ".tif":
