...
def onnx_converter(onnx_model_path:str,  output_path:str=None, 
    ...
    """
    # 确保 target_formats 是列表类型，并包含 'keras' 或 'tflite'
    ...
    # 4. 处理输出路径
    onnx_path, model_name = os.path.split(onnx_model_path) # 获取 ONNX 模型所在目录
    if output_path is None:
        output_path = onnx_path  # 若未提供 output_path，则默认保存在 ONNX 所在目录
    output_path = os.path.join(output_path, model_name.split('.')[0]) # 设置输出文件的基本路径

    # 5. 如果是 FP16 或 INT8 量化模型，修改输出路径
    if fp16_model:
