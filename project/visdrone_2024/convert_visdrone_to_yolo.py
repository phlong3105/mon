...
@click.option("--image-dir",  type=click.Path(exists=True),  default=mon.DATA_DIR/"visdrone/visdrone_2024_det/test_dev/images",      help="Image directory.")
@click.option("--label-dir",  type=click.Path(exists=True),  default=mon.DATA_DIR/"visdrone/visdrone_2024_det/test_dev/annotations", help="Bounding bbox directory.")
@click.option("--output-dir", type=click.Path(exists=False), default=None, help="Output directory.")
@click.option("--include-ignore", is_flag=True)
@click.option("--include-other",  is_flag=True)
	...
	with mon.get_progress_bar() as pbar:
		for i in pbar.track(
			...
			if label_file.is_txt_file():
				...
				b = np.array([list(map(float, x[0:4])) for x in l])  # bbox
			...
			with open(output_file, "w") as out_file:
				for j, x in enumerate(l):
					c = int(x[5])  # class
					if include_ignore and c == 0:
						out_file.write(f"{c} {b[j][0]:.6f} {b[j][1]:.6f} {b[j][2]:.6f} {b[j][3]:.6f}\n")
					elif include_other and c == 11:
						out_file.write(f"{c} {b[j][0]:.6f} {b[j][1]:.6f} {b[j][2]:.6f} {b[j][3]:.6f}\n")
					else:
						if c in [0, 11]:
							continue
						else:
							out_file.write(f"{c - 1} {b[j][0]:.6f} {b[j][1]:.6f} {b[j][2]:.6f} {b[j][3]:.6f}\n")
