...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        # Add one xyxy bbox to image with label
        if isinstance(box, torch.Tensor):
            box = box.tolist()
        if self.pil or not is_ascii(label):
            self.draw.rectangle(box, width=self.lw, outline=color)  # bbox
            if label:
                if self.pil_9_2_0_check:
                    _, _, w, h = self.font.getbbox(label)  # text width, height (New)
                else:
                    w, h = self.font.getsize(label)  # text width, height (Old, deprecated in 9.2.0)
                outside = box[1] - h >= 0  # label fits outside bbox
                self.draw.rectangle(
                    (box[0], box[1] - h if outside else box[1], box[0] + w + 1,
                     box[1] + 1 if outside else box[1] + h + 1),
                    fill=color,
                )
                # self.draw.text((bbox[0], bbox[1]), label, fill=txt_color, font=self.font, anchor='ls')  # for PIL>8.0
    ...
    def masks(self, masks, colors, im_gpu, alpha=0.5, retina_masks=False):
        ...
        colors = torch.tensor(colors, device=im_gpu.devices, dtype=torch.float32) / 255.0
...
def save_one_box(xyxy, im, file=Path('im.jpg'), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    b[:, 2:] = b[:, 2:] * gain + pad  # bbox wh * gain + pad
