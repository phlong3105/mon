...
class Exporter:
    ...
    def __call__(self, model=None):
        ...
        # Load PyTorch model
        self.device = select_device('cpu' if self.args.devices is None else self.args.devices)
        if self.args.half:
    ...
    def _export_engine(self, workspace=4, verbose=False, prefix=colorstr('TensorRT:')):
        ...
        assert self.im.devices.type != 'cpu', "export running on CPU but must be on GPU, i.e. use 'device=0'"
        try:
