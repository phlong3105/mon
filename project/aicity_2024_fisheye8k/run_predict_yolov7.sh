...
yolov7_dir="${mon_dir}/src/mon_extra/vision/detect/yolov7"
...
for ((i=0; i < ${#test_cameras[@]}; i++)); do
    ...
    python -W ignore my_predict.py \
      --root "${current_dir}" \
      --config "${current_dir}/config/yolov7_e6e_aicity_2024_fisheye8k_1920.yaml" \
      --weights \
        "${current_dir}/run/train/yolov7_e6e_aicity_2024_fisheye8k_1920_epoch_200/weights/best_f1.pt,
         ${current_dir}/run/train/yolov7_e6e_aicity_2024_fisheye8k_1536_epoch_200/weights/best_f1.pt,
         ${current_dir}/run/train/yolov7_e6e_aicity_2024_fisheye8k_1280_epoch_200/weights/best_f1.pt"\
      --model "yolov7_e6e" \
      --data "${source}" \
      --save-dir "${save_dir}" \
      --device "cuda:0" \
