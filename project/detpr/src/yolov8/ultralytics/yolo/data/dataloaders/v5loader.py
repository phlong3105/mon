...
from ultralytics.yolo.utils import (
    DATASETS_DIR, is_colab, is_dir_writeable, is_kaggle, LOGGER, NUM_THREADS,
    TQDM_BAR_FORMAT, yaml_load,
)
...
IMG_FORMATS = 'bmp', 'dng', 'jpeg', 'jpg', 'mpo', 'png', 'tif', 'tiff', \
    'webp', 'pfm'  # include image suffixes
VID_FORMATS = 'asf', 'avi', 'gif', 'm4v', 'mkv', 'mov', 'mp4', 'mpeg', 'mpg', \
    'ts', 'wmv'  # include video suffixes
LOCAL_RANK = int(
    os.getenv('LOCAL_RANK', -1)
)  # https://pytorch.org/docs/stable/elastic/run.html
RANK = int(os.getenv('RANK', -1))
PIN_MEMORY = str(
    os.getenv('PIN_MEMORY', True)
).lower() == 'true'  # global pin_memory for dataloaders
...
def exif_transpose(image):
    ...
    Inplace version of https://github.com/python-pillow/Pillow/blob/master
    /src/PIL/ImageOps.py exif_transpose()
...
def create_dataloader(
    ...
    if rect and shuffle:
        LOGGER.warning(
            'WARNING ⚠️ --rect is incompatible with DataLoader shuffle, '
    ...
    loader = DataLoader if image_weights or close_mosaic else \
        InfiniteDataLoader  # DataLoader allows attribute updates
...
class LoadScreenshots:
    # YOLOv5 screenshot dataloader, i.e. `python detect.py --source "screen 0
    # 100 100 512 256"`
    def __init__(
        self,
        source,
    ...
    def __next__(self):
        ...
        im0 = np.array(self.sct.grab(self.monitor))[:, :,
              :3]  # [:, :, :3] BGRA to BGR
        s = f"screen {self.screen} (LTWH): {self.left},{self.top}," \
        ...
        if self.transforms:
            ...
            im = \
            letterbox(im0, self.img_size, stride=self.stride, auto=self.auto)[
                0]  # padded resize
        ...
        return str(
            self.screen
        ), im, im0, None, s  # screen, img, original img, im0s, s
...
class LoadImages:
    ...
    def __init__(
        ...
        if isinstance(path, str) and Path(
            path
        ).suffix == ".txt":  # *.txt file with img/vid/dir on each line
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            self.frame += 1
            # im0 = self._cv2_rotate(im0)  # for use if cv2 autorotation is
            # False
            s = f'video {self.count + 1}/{self.nf} ({self.frame}/' \
                f'{self.frames}) {path}: '
    ...
    def _new_video(self, path):
        ...
        # https://github.com/ultralytics/yolov5/issues/8493
...
class LoadStreams:
    ...
    def __init__(
        ...
        self.stride = stride
        self.vid_stride = vid_stride  # video frame-rate stride
        sources = Path(sources).read_text().rsplit() if os.path.isfile(
            sources
        ) else [sources]
        n = len(sources)
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            if urlparse(s).hostname in ('www.youtube.com', 'youtube.com',
                                        'youtu.be'):  # if source is YouTube
                # video
                # YouTube format i.e.
                # 'https://www.youtube.com/watch?v=Zgi9g1ksQHc' or
            ...
            if s == 0:
                assert not is_colab(), '--source 0 webcam unsupported on ' \
                                       'Colab. Rerun command in a local ' \
                                       'environment.'
                assert not is_kaggle(), '--source 0 webcam unsupported on ' \
                                        'Kaggle. Rerun command in a local ' \
                                        'environment.'
            ...
            self.fps[i] = max(
                (fps if math.isfinite(fps) else 0) % 100,
                0
            ...
            LOGGER.info(
                f"{st} Success ({self.frames[i]} frames {w}x{h} at "
                f"{self.fps[i]:.2f} FPS)"
            )
        ...
        s = np.stack(
            [letterbox(x, img_size, stride=stride, auto=auto)[0].shape for x in
             self.imgs]
        )
        ...
        if not self.rect:
            LOGGER.warning(
                'WARNING ⚠️ Stream shapes differ. For optimal performance '
                'supply similarly-shaped streams.'
            )
...
class LoadImagesAndLabels(Dataset):
    # YOLOv5 train_loader/val_loader, loads images and labels for training
    # and validation
    ...
    def __init__(
        ...
        self.mosaic = self.augment and not self.rect  # load 4 images at a
        # time into a mosaic (only during training)
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                if p.is_dir():  # dir
                    ...
                    with open(p) as t:
                        ...
                        # f += [p.parent / x.lstrip(os.sep) for x in t]  # to
                        # global path (pathlib)
                else:
                    raise FileNotFoundError(f'{prefix}{p} does not exist')
            self.im_files = sorted(
                x.replace('/', os.sep) for x in f if
                x.split('.')[-1].lower() in IMG_FORMATS
            )
            # self.img_files = sorted([x for x in f if x.suffix[1:].lower()
            # in IMG_FORMATS])  # pathlib
        ...
        try:
            ...
            assert cache[
                       'version'] == self.cache_version  # matches current
            # version
        ...
        assert nf > 0 or not augment, f'{prefix}No labels found' \
                                      f' in {cache_path}, can not start ' \
                                      f'training. {HELP_URL}'
        ...
        assert nl > 0 or not augment, f'{prefix}All labels empty in ' \
                                      f'{cache_path}, can not start training.' \
                                      f' {HELP_URL}'
        ...
        if min_items:
            include = \
            np.array([len(x) >= min_items for x in self.labels]).nonzero()[
                0].astype(int)
            LOGGER.info(
        ...
        # Update labels
        include_class = []  # filter labels to include only these classes (
        # optional)
        ...
        if cache_images:
            ...
            fcn = self.cache_images_to_disk if cache_images == 'disk' else \
                self.load_image
                ...
                for i, x in pbar:
                    if cache_images == 'disk':
                        ...
                        self.ims[i], self.im_hw0[i], self.im_hw[
                            i] = x  # im, hw_orig, hw_resized = load_image(
                        # self, i)
                        b += self.ims[i].nbytes
    ...
    def check_cache_ram(self, safety_margin=0.1, prefix=''):
        ...
        cache = mem_required * (
                1 + safety_margin) < mem.available  # to cache or not to
        # cache, that is the question
        if not cache:
            LOGGER.info(
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        x = {}  # dict
        nm, nf, ne, nc, msgs = 0, 0, 0, 0, []  # number missing, found,
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ...
            for im_file, lb, shape, segments, nm_f, nf_f, ne_f, nc_f, msg in \
                pbar:
                nm += nm_f
                nf += nf_f
                ne += ne_f
        ...
        if is_dir_writeable(path.parent):
            ...
            LOGGER.warning(
                f'{prefix}WARNING ⚠️ Cache directory {path.parent} is not '
                f'writeable'
            )  # not writeable
        return x
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            shape = self.batch_shapes[self.batch[
                index]] if self.rect else self.img_size  # final letterboxed
            # shape
    ...
    def load_mosaic(self, index):
        # YOLOv5 4-mosaic loader. Loads 1 image + 3 random images into a
        # 4-image mosaic
        labels4, segments4 = [], []
        s = self.img_size
        ...
        for i, index in enumerate(indices):
            ...
            if i == 0:  # top left
                ...
                x1a, y1a, x2a, y2a = max(xc - w, 0), max(
                    yc - h,
                    0
                    ), xc, yc  # xmin, ymin, xmax, ymax (large image)
                ...
                x1a, y1a, x2a, y2a = xc, yc, min(xc + w, s * 2), min(
                    s * 2,
                    yc + h
                    )
            ...
            # Labels
            labels, segments = self.labels[index].copy(), self.segments[
                index].copy()
            if labels.size:
                labels[:, 1:] = xywhn2xyxy(
                    labels[:, 1:],
    ...
    def load_mosaic9(self, index):
        # YOLOv5 9-mosaic loader. Loads 1 image + 8 random images into a
        # 9-image mosaic
        labels9, segments9 = [], []
        s = self.img_size
        indices = [index] + random.choices(
        ...
        # Offset
        yc, xc = (int(random.uniform(0, s)) for _ in
                  self.mosaic_border)  # mosaic center x, y
...
def extract_boxes(
    ...
    # Convert detection dataset into classification dataset, with one
    # directory per class
    path = Path(path)  # images dir
    shutil.rmtree(path / 'classification') if (
            path / 'classification').is_dir() else None  # remove existing
...
def autosplit(
    ...
    print(
        f'Autosplitting images from {path}' + ', using *.txt labeled images '
                                              'only' * annotated_only
        )
    for i, img in tqdm(zip(indices, files), total=n):
        if not annotated_only or Path(
            img2label_paths([str(img)])[0]
        ).exists():  # check label
...
def verify_image_label(args):
    ...
    try:
        ...
        if im.format.lower() in ('jpg', 'jpeg'):
            with open(im_file, 'rb') as f:
                ...
                if f.read() != b'\xff\xd9':  # corrupt JPEG
                    ...
                    msg = f'{prefix}WARNING ⚠️ {im_file}: corrupt JPEG ' \
                          f'restored and saved'
        ...
        if os.path.isfile(lb_file):
            ...
            if nl:
                assert lb.shape[
                           1] == 5, f'labels require 5 columns, {lb.shape[1]} ' \
                                    f'columns detected'
                assert (lb >= 0).all(), f'negative label values {lb[lb < 0]}'
                assert (lb[:,
                        1:] <= 1).all(), f'non-normalized or out of bounds ' \
                                         f'coordinates {lb[:, 1:][lb[:, 1:] > 1]}'
                _, i = np.unique(lb, axis=0, return_index=True)
                if len(i) < nl:  # duplicate row check
                    lb = lb[i]  # remove duplicates
                    if segments:
                        segments = [segments[x] for x in i]
                    msg = f'{prefix}WARNING ⚠️ {im_file}: {nl - len(i)} ' \
                          f'duplicate labels removed'
            else:
                ne = 1  # label empty
...
class HUBDatasetStats():
    ...
    Arguments
        path:           Path to data.yaml or data.zip (with data.yaml inside
        data.zip)
    ...
    def __init__(self, path='coco128.yaml', autodownload=False):
        ...
        self.stats = {
            'nc': data['nc'], 'names': list(data['names'].values())
    ...
    def _find_yaml(dir):
        ...
        if len(files) > 1:
            files = [f for f in files if
                     f.stem == dir.stem]  # prefer *.yaml files that match
            # dir name
            assert files, f'Multiple *.yaml files found in {dir}, only 1 ' \
                          f'*.yaml file allowed'
        assert len(
            files
        ) == 1, f'Multiple *.yaml files found: {files}, only 1 *.yaml fi' \
                f'le allowed in {dir}'
        return files[0]
    ...
    def _unzip(self, path):
        ...
        assert dir.is_dir(), f'Error unzipping {path}, {dir} not found. ' \
                             f'path/to/abc.zip MUST unzip to path/to/abc/'
        return True, str(dir), self._find_yaml(
            dir
...
class ClassificationDataset(torchvision.datasets.ImageFolder):
    ...
    def __init__(self, root, augment, imgsz, cache=False):
        ...
        self.samples = [list(x) + [Path(x[0]).with_suffix('.npy'), None] for x
                        in self.samples]  # file, index, npy, im
    ...
    def __getitem__(self, i):
        ...
        if self.album_transforms:
            sample = \
            self.album_transforms(image=cv2.cvtColor(im, cv2.COLOR_BGR2RGB))[
                "image"]
        else:
