...
def normalize(x, mean=IMAGENET_MEAN, std=IMAGENET_STD, inplace=False):
    # Denormalize RGB images x per ImageNet stats in BCHW format, i.e. = (x -
    # mean) / std
...
def augment_hsv(im, hgain=0.5, sgain=0.5, vgain=0.5):
    ...
    if hgain or sgain or vgain:
        r = np.random.uniform(-1, 1, 3) * [hgain, sgain,
                                           vgain] + 1  # random gains
...
def copy_paste(im, labels, segments, p=0.5):
    ...
    if p and n:
        ...
        boxes = np.stack(
            [w - labels[:, 3], labels[:, 2], w - labels[:, 1], labels[:, 4]],
            axis=-1
        )  # (n, 4)
...
def classify_albumentations(
    ...
    # YOLOv5 classification Albumentations (optional, only used if package is
    # installed)
    prefix = colorstr('albumentations: ')
    try:
        import albumentations as A
        ...
        if augment:  # Resize and crop
            ...
            if auto_aug:
                ...
                if jitter > 0:
                    color_jitter = (float(
                        jitter
                    ),) * 3  # repeat value for brightness, contrast,
                    # satuaration, 0 hue
        ...
        LOGGER.warning(
            f'{prefix}⚠️ not found, install with `pip install albumentations` '
            f'(recommended)'
        )
    except Exception as e:
        LOGGER.info(f'{prefix}{e}')
...
class LetterBox:
    # YOLOv5 LetterBox class for image preprocessing, i.e. T.Compose([
    # LetterBox(size), ToTensor()])
