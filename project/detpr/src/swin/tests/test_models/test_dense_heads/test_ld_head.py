...
def test_ld_head_loss():
    ...
    # be non-negative but there should be no bbox loss.
    empty_cls_loss = sum(empty_gt_losses['loss_cls'])
    empty_box_loss = sum(empty_gt_losses['loss_bbox'])
    empty_ld_loss = sum(empty_gt_losses['loss_ld'])
    assert empty_cls_loss.item() > 0, 'cls loss should be non-zero'
    assert empty_box_loss.item() == 0, (
        'there should be no bbox loss when there are no true boxes')
    assert empty_ld_loss.item() >= 0, 'ld loss should be non-negative'

    # When truth is non-empty then both cls and bbox loss should be nonzero
    # for random inputs
    gt_bboxes = [
    ...
    # When truth is non-empty and not ignored then both cls and bbox loss should
