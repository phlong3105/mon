...
def test_sabl_retina_head_loss():
    ...
    if torch.cuda.is_available():
        ...
        assert empty_box_cls_loss.item() == 0, (
            'there should be no bbox cls loss when there are no true boxes')
        assert empty_box_reg_loss.item() == 0, (
            'there should be no bbox reg loss when there are no true boxes')

        # When truth is non-empty then both cls and bbox loss should
        ...
        assert onegt_box_cls_loss.item() > 0, 'bbox loss cls should be non-zero'
        assert onegt_box_reg_loss.item() > 0, 'bbox loss reg should be non-zero'
