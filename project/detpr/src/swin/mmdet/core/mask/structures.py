...
class BaseInstanceMasks(metaclass=ABCMeta):
    ...
    def crop(self, box):
        ...
        Args:
            box (ndarray): Bbox in format [x1, y1, x2, y2], shape (4, ).
    ...
    def crop_and_resize(
        ...
        assigned box and resize to the size of (mask_h, mask_w)

        Args:
            bboxes (Tensor): Bboxes in format [x1, y1, x2, y2], shape (N, 4)
            out_shape (tuple[int]): Target (h, w) of resized mask
            inds (ndarray): Indexes to assign masks to each box,
...
class BitmapMasks(BaseInstanceMasks):
    ...
    def crop(self, box):
        ...
        # clip the boundary
        box = box.copy()
        box[0::2] = np.clip(box[0::2], 0, self.width)
        box[1::2] = np.clip(box[1::2], 0, self.height)
        x1, y1, x2, y2 = box
    ...
    def crop_and_resize(
        ...
        fake_inds = torch.arange(
            num_bbox, device=device
        ).to(dtype=bboxes.dtype)[:, None]
        ...
        if num_bbox > 0:
            ...
            targets = roi_align(
                gt_masks_th[:, None, :, :], rois, out_shape,
                1.0, 0, 'avg', True
            ).squeeze(1)
...
class PolygonMasks(BaseInstanceMasks):
    ...
    def translate(
        ...
        Example:
            ...
            >>> assert np.all(new.masks[0][0][0::2] == self.masks[0][0][0::2]
            + 4)  # noqa: E501
        """
        assert fill_val is None or fill_val == 0, 'Here fill_val is not ' \
                                                  f'used, and defaultly ' \
                                                  f'should be None or 0. got ' \
                                                  f'{fill_val}.'
        if len(self.masks) == 0:
    ...
    def areas(self):
        ...
        /ffff8acc35ea88ad1cb1806ab0f00b4c1c5dbfd9/detectron2/structures/masks
        .py#L387>`_.
    ...
    def _polygon_area(self, x, y):
        ...
        https://stackoverflow.com/questions/24467972/calculate-area-of
        -polygon-given-x-y-coordinates
    ...
    def random(
        ...
        References:
            .. [1] https://gitlab.kitware.com/computer-vision/kwimage/-/blob
            /928cae35ca8/kwimage/structs/polygon.py#L379  # noqa: E501
        ...
        def _order_vertices(verts):
            """
            References:
                https://stackoverflow.com/questions/1709283/how-can-i-sort-a
                -coordinate-list-for-a-rectangle-counterclockwise
            """
