...
class GridAssigner(BaseAssigner):
    ...
    def assign(self, bboxes, box_responsible_flags, gt_bboxes, gt_labels=None):
        ...
        3. for each box within a cell, if the iou with its nearest gt >
            pos_iou_thr and the center of that gt falls inside the cell,
            assign it to that box
        4. for each gt box, assign its nearest proposals within the cell the
            gt box falls in to itself.
