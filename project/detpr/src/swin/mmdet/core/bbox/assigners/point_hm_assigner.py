...
class PointHMAssigner(BaseAssigner):
    """Assign a corresponding gt box or background to each point.
    ...
    def assign(self, points, gt_bboxes, gt_labels=None):
        ...
        This method assign a gt box to every point, each box
        ...
        2. for each gt box, we find the k most closest points to the
            box center and assign the gt box to those points, we also record
            the minimum distance from each point to the closest gt box. When we
            assign the box to the points, we check whether its distance to the
            points is closest.
        ...
        if num_points == 0 or num_gts == 0:
            ...
            assigned_gt_offset_tl = points.new_zeros(
                (num_points, 2),
                dtype=torch.float32
            )
            assigned_gt_offset_br = points.new_zeros(
                (num_points, 2),
                dtype=torch.float32
            )
            
            pos_inds_tl = torch.nonzero(
                assigned_gt_hm_tl == 1,
                as_tuple=False
                ).squeeze(-1).unique()
            pos_inds_br = torch.nonzero(
                assigned_gt_hm_br == 1,
                as_tuple=False
                ).squeeze(-1).unique()
            neg_inds_tl = torch.nonzero(
                assigned_gt_hm_tl < 1,
                as_tuple=False
                ).squeeze(-1).unique()
            neg_inds_br = torch.nonzero(
                assigned_gt_hm_br < 1,
                as_tuple=False
                ).squeeze(-1).unique()
            
            return assigned_gt_hm_tl, assigned_gt_offset_tl, pos_inds_tl, \
                neg_inds_tl, \
                assigned_gt_hm_br, assigned_gt_offset_br, pos_inds_br, \
                neg_inds_br
        ...
        # assign gt box
        gt_bboxes_xtl, gt_bboxes_ytl, gt_bboxes_xbr, gt_bboxes_ybr = \
            torch.chunk(
            gt_bboxes,
            4,
            dim=1
        ...
        distances_tl = (
                points_xy[:, None, :] - gt_bboxes_xytl[None, :, :]).norm(dim=2)
        distances_br = (
                points_xy[:, None, :] - gt_bboxes_xybr[None, :, :]).norm(dim=2)
        ...
        lvls = torch.arange(
            lvl_min,
            lvl_max + 1,
            dtype=points_lvl.dtype,
            device=points_lvl.devices
            )
        for gt_lvl in lvls:
            ...
            assigned_gt_offset_tl[min_dist_points_index_tl, :] = \
                (gt_bboxes_xytl - lvl_points[min_dist_index_tl,
                                  :]) / downscale_factor
            ...
            assigned_gt_offset_br[min_dist_points_index_br, :] = \
                (gt_bboxes_xybr - lvl_points[min_dist_index_br,
                                  :]) / downscale_factor
            if self.gaussian_bump:
                out_index_tl = lvl_distances_tl >= radius[None, :]
                lvl_gaussian_tl = torch.exp(
                    -torch.pow(lvl_distances_tl, 2) / (2 * sigma * sigma)[None,
                                                      :]
                    )
                lvl_gaussian_tl[out_index_tl] = -INF
                max_gaussian_tl, _ = lvl_gaussian_tl.max(dim=1)
                assigned_gt_hm_tl[points_index[max_gaussian_tl != -INF]] = \
                max_gaussian_tl[max_gaussian_tl != -INF]
                
                out_index_br = lvl_distances_br >= radius[None, :]
                lvl_gaussian_br = torch.exp(
                    -torch.pow(lvl_distances_br, 2) / (2 * sigma * sigma)[None,
                                                      :]
                    )
                lvl_gaussian_br[out_index_br] = -INF
                max_gaussian_br, _ = lvl_gaussian_br.max(dim=1)
                assigned_gt_hm_br[points_index[max_gaussian_br != -INF]] = \
                max_gaussian_br[max_gaussian_br != -INF]
