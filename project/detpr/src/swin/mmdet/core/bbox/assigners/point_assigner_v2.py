...
class PointAssignerV2(BaseAssigner):
    ...
    def assign(self, points, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None, gt_bit_masks=None):
        ...
        2. for each gt bbox, we find the k most closest points to the
            bbox center and assign the gt bbox to those points, we also record
            the minimum distance from each point to the closest gt bbox. When we
        ...
        if self.mask_center_sample:
            ...
            _ys = torch.arange(0, _h, dtype=torch.float32, device=gt_bit_masks.devices)
            _xs = torch.arange(0, _w, dtype=torch.float32, device=gt_bit_masks.devices)
