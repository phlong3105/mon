...
class TridentRoIHead(StandardRoIHead):
    ...
    def merge_trident_bboxes(self, trident_det_bboxes, trident_det_labels):
        """Merge box predictions of each branch."""
        if trident_det_bboxes.numel() == 0:
            ...
            det_bboxes, keep = batched_nms(
                nms_bboxes, nms_scores, nms_inds,
                nms_cfg
            )
    ...
    def simple_test(
        ...
        1. Compute prediction box and label per branch.
        ...
        for i in range(len(img_metas) // num_branch):
            det_result = self.merge_trident_bboxes(
                torch.cat(
                    det_bboxes_list[i * num_branch:(i + 1) *
                                                   num_branch]
                ),
                torch.cat(
                    det_labels_list[i * num_branch:(i + 1) *
                                                   num_branch]
                )
            )
