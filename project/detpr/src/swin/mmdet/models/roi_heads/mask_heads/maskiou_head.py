...
class MaskIoUHead(nn.Module):
    ...
    def get_targets(
        self, sampling_results, gt_masks, mask_pred, mask_targets,
        rcnn_train_cfg
    ):
        """Compute target of mask IoU.

        Mask IoU target is the IoU of the predicted mask (inside a box) and
        the gt mask of corresponding gt mask (the whole instance).
        The intersection area is computed inside the box, and the gt mask area
        is computed with two steps, firstly we compute the gt area inside the
        box, then divide it by the area ratio of gt area inside the box and
        ...
        # the whole instance
        area_ratios = map(
            self._get_area_ratio, pos_proposals,
    ...
    def _get_area_ratio(self, pos_proposals, pos_assigned_gt_inds, gt_masks):
        ...
        if num_pos > 0:
            ...
            for i in range(num_pos):
                ...
                box = proposals_np[i, :].astype(np.int32)
                gt_mask_in_proposal = gt_mask.crop(box)
                
                ratio = gt_mask_in_proposal.areas[0] / (
                    gt_instance_mask_area[pos_assigned_gt_inds[i]] + 1e-7)
                area_ratios.append(ratio)
            area_ratios = torch.from_numpy(np.stack(area_ratios)).float().to(
                pos_proposals.devices
            )
        else:
