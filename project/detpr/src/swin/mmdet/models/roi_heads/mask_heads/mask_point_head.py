...
class MaskPointHead(nn.Module):
    ...
    def _get_target_single(self, rois, rel_roi_points, pos_assigned_gt_inds,
        ...
        if num_pos > 0:
            gt_masks_th = (
                gt_masks.to_tensor(rois.dtype, rois.devices).index_select(
    ...
    def _get_uncertainty(self, mask_pred, labels):
        ...
        if mask_pred.shape[1] == 1:
            ...
            inds = torch.arange(mask_pred.shape[0], device=mask_pred.devices)
    ...
    def get_roi_rel_points_train(self, mask_pred, labels, cfg):
        ...
        point_coords = torch.rand(
            batch_size, num_sampled, 2, device=mask_pred.devices)
        ...
        shift = num_sampled * torch.arange(
            batch_size, dtype=torch.long, device=mask_pred.devices)
        idx += shift[:, None]
        point_coords = point_coords.view(-1, 2)[idx.view(-1), :].view(
            batch_size, num_uncertain_points, 2)
        if num_random_points > 0:
            rand_roi_coords = torch.rand(
                batch_size, num_random_points, 2, device=mask_pred.devices)
