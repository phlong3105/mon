...
from mmcv.cnn import bias_init_with_prob, ConvModule, normal_init, Scale
...
class VFNetHead(ATSSHead, FCOSHead):
    ...
    Args:
        ...
        gradient_mul (float): The multiplier to gradients from box refinement
            and recognition. Default: 0.1.
        bbox_norm_type (str): The box normalization type, 'reg_denom' or
    ...
    def forward_single(self, x, scale, scale_refine, stride, reg_denom):
        ...
        Args:
            ...
            scale_refine (:obj: `mmcv.cnn.Scale`): Learnable scale module to
                resize the refined box prediction.
            stride (int): The corresponding stride for feature maps,
                used to normalize the box prediction when
                bbox_norm_type = 'stride'.
            reg_denom (int): The corresponding regression range for feature
                maps, only used to normalize the box prediction when
                bbox_norm_type = 'reg_denom'.

        Returns:
            tuple: iou-aware cls scores for each box, box predictions and
                refined box predictions of input feature maps.
        """
        cls_feat = x
        reg_feat = x
    ...
    def star_dcn_offset(self, bbox_pred, gradient_mul, stride):
        ...
        Args:
            bbox_pred (Tensor): Predicted box distance offsets (l, r, t, b).
            gradient_mul (float): Gradient multiplier.
            stride (int): The corresponding stride for feature maps,
                used to project the box onto the feature map.
    ...
    def get_bboxes(
        ...
        """Transform network outputs for a batch into box predictions.
    ...
    def _get_bboxes_single(
        ...
        """Transform outputs for a single batch item into box predictions.
    ...
    def get_targets(
        self, cls_scores, mlvl_points, gt_bboxes, gt_labels,
        ...
        if self.use_atss:
            return self.get_atss_targets(
                cls_scores, mlvl_points, gt_bboxes,
                gt_labels, img_metas,
                gt_bboxes_ignore
            )
        else:
