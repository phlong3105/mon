...
from mmdet.models.utils import (
    build_positional_encoding, build_transformer, FFN,
)
...
class TransformerHead(AnchorFreeHead):
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert not use_sigmoid_cls, 'setting use_sigmoid_cls as True is ' \
                                    'not supported in DETR, since background ' \
                                    'is needed for the ' \
                                    'matching process.'
    ...
    def forward_single(self, x, img_metas):
        ...
        outs_dec, _ = self.transformer(
            x, masks, self.query_embedding.weight,
            pos_embed
        )
    ...
    def _get_bboxes_single(
        ...
        """Transform outputs from the last decoder layer into box predictions
        for each image.
