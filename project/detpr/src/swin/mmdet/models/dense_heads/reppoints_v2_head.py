...
from mmcv.ops import CornerPool, DeformConv2d
from mmdet.core import (
    build_assigner, build_sampler, images_to_levels, multi_apply,
    multiclass_nms_rpd, PointGenerator, unmap,
)
from mmdet.utils.common import compute_locations
from mmdet.utils.instances import Instances

from .anchor_free_head import AnchorFreeHead
from ..builder import build_head, build_loss, HEADS
...
class RepPointsV2Head(AnchorFreeHead):
    ...
    Args:
        ...
        point_base_scale (int): box scale for assigning labels.
        ...
        transform_method (str): The methods to transform RepPoints to box.
    """  # noqa: W605
    ...
    def _init_layers(self):
        ...
        self.reppoints_sem_out = nn.Conv2d(
            self.feat_channels,
    ...
    def points2bbox(self, pts, y_first=True):
        ...
        :return: each points set is converting to a box [x1, y1, x2, y2].
        """
    ...
    def gen_grid_from_reg(self, reg, previous_boxes):
        ...
        regressed_bbox = torch.cat(
            [
                grid_left, grid_top, grid_left + grid_width,
    ...
    def forward(self, feats):
        if not self.mask_head:
            ...
            bbox_res = multi_apply(
                self.forward_single,
                feats,
                controller=self.controller
            )  # cls_out, pts_out_init, pts_out_refine, controller_out
            device = bbox_res[1][0].devices
            self.level_sum = tuple(
                accumulate([0] + [r[:, 0].numel() for r in bbox_res[1]][:-1])
            )
            self.image_sum = torch.tensor(
                [r[0, 0].numel() for r in bbox_res[1]]
            )
            self.pred_instances.fpn_levels = torch.cat(
                [torch.tensor([i] * r[:, 0].numel()).to(device) for i, r in
                 enumerate(bbox_res[1])]
            )
            self.pred_instances.im_inds = torch.cat(
                [torch.cat(
                    [torch.tensor([i] * img.numel()).to(device) for i, img in
                     enumerate(r[:, 0])]
                ) for r in bbox_res[1]]
            )
            ...
            pts_grid = torch.cat(
                [compute_locations(
                    *r.shape[-2:],
                    self.point_strides[level],
                    device
                ).reshape(-1, 2).repeat(len(r), 1) for level, r in
                 enumerate(bbox_res[1])],
                dim=0
            )
            pts_center_list = []
            for i_lvl in range(len(self.point_strides)):
                ...
                yx_pts_shift = pts_shift.reshape(
                    *pts_shift.shape[:-1],
                    2 * self.num_points
                    )
            ...
            self.pred_instances.locations = torch.cat(
                [l.flatten(0, -2) for l in pts_center_list]
            ) + pts_grid
            
            self.pred_instances.mask_head_params = torch.cat(
                [r.permute(0, 2, 3, 1).flatten(0, -2) for r in bbox_res[-1]]
            )
            return bbox_res[:-1]
    ...
    def forward_single(self, x, controller=None):
        ...
        # If we use bounding box representation, the initial reppoints is
        #   from regular grid placed on a pre-defined box.
        ...
        hem_offset_out = torch.cat(
            [hem_tl_offset_out, hem_br_offset_out],
            dim=1
        )
        
        # initialize reppoints
        # pts_out_init = self.reppoints_pts_init_out(self.relu(
        # self.reppoints_pts_init_conv(pts_feat)))
        ...
        pts_out_init_grad_mul = (
                                        1 - self.gradient_mul) * \
                                pts_out_init.detach() + self.gradient_mul * \
                                pts_out_init
        ...
        if self.mask_head and controller is not None and self.controller_on \
            == 'cls':
            controller_out = controller(cls_tower)
        refine_reg_tower = self.relu(
            self.reppoints_pts_refine_conv(pts_feat, dcn_offset)
        )
        ...
        if self.mask_head:
            return cls_out, pts_out_init, pts_out_refine, hem_score_out, \
                hem_offset_out, sem_scores_out, controller_out
        else:
            return cls_out, pts_out_init, pts_out_refine, hem_score_out, \
    ...
    def get_points(self, featmap_sizes, img_metas):
        ...
        points_list = [[point.clone() for point in multi_level_points] for _ in
                       range(num_imgs)]
    ...
    def centers_to_bboxes(self, point_list):
        ...
        for i_img, point in enumerate(point_list):
            ...
            for i_lvl in range(len(self.point_strides)):
                ...
                box.append(bbox_center + bbox_shift)
    ...
    def _point_target_single(
        ...
        if unmap_outputs:
            ...
            gt_inds = unmap(
                sampling_result.assign_result.gt_inds,
                num_total_proposals,
                inside_flags
            )
        
        return labels, label_weights, bbox_gt, bbox_weights, pos_inds, \
            neg_inds, gt_inds
    ...
    def get_targets(
        ...
        Returns:
            tuple:
                ...
                level.  # noqa: E501
                - bbox_gt_list (list[Tensor]): Ground truth box of each level.
                - proposal_list (list[Tensor]): Proposals(points/bboxes) of
                each level.  # noqa: E501
        ...
        return (
        labels_list, label_weights_list, bbox_gt_list, bbox_weights_list,
        num_total_pos, num_total_neg, shift_gt_inds)
    
    def _hm_target_single(
        self,
        flat_points,
        inside_flags,
        gt_bboxes,
        ...
        return (
        gt_hm_tl, gt_offset_tl, hm_tl_weights, offset_tl_weights, pos_inds_tl,
        neg_inds_tl,
        gt_hm_br, gt_offset_br, hm_br_weights, offset_br_weights, pos_inds_br,
        neg_inds_br)
    ...
    def get_hm_targets(
        ...
        if gt_labels_list is None:
            gt_labels_list = [None for _ in range(num_imgs)]
        (all_gt_hm_tl, all_gt_offset_tl, all_hm_tl_weights,
         all_offset_tl_weights, pos_inds_tl_list, neg_inds_tl_list,
         all_gt_hm_br, all_gt_offset_br, all_hm_br_weights,
         all_offset_br_weights, pos_inds_br_list, neg_inds_br_list) = \
            multi_apply(
        ...
        return (gt_hm_tl_list, gt_offset_tl_list, hm_tl_weight_list,
                ...
                gt_hm_br_list, gt_offset_br_list, hm_br_weight_list,
                offset_br_weight_list,
                num_total_pos_tl, num_total_neg_tl, num_total_pos_br,
                num_total_neg_br)
    
    def loss_single(
        self, cls_score, pts_pred_init, pts_pred_refine, hm_score, hm_offset,
        ...
        return loss_cls, loss_pts_init, loss_pts_refine, loss_heatmap, \
            loss_offset
    ...
    def loss(
        ...
        if self.train_cfg.init.assigner['type'] != 'MaxIoUAssigner':
            ...
            # transform center list to box list and
            #   assign target for box list
        ...
        (gt_hm_tl_list, gt_offset_tl_list, gt_hm_tl_weight_list,
         ...
         gt_hm_br_list, gt_offset_br_list, gt_hm_br_weight_list,
        ...
        for i_img, center in enumerate(center_list):
            ...
            for i_lvl in range(len(pts_preds_refine)):
                ...
                box.append(
                    bbox_center + bbox_shift[i_img].permute(1, 2, 0).reshape(
                        -1,
                        4
                    )
                    )
        ...
        (labels_list, label_weights_list,
         ...
         gt_inds) = cls_reg_targets_refine
        ...
        if self.mask_head:
            ...
            self.pred_instances.gt_inds = torch.cat(
                [torch.flatten(gt_ind) for gt_ind in gt_inds]
            )
            # norm by gt box, size=(W,H)
            self.pred_instances.boxsz = torch.cat(
                [get_shape(gt_bbox.reshape(-1, 4)) for gt_bbox in
                 bbox_gt_list_refine]
            )
            # norm by pred box, size=(W,H)
            # bbox_pred_refine = [self.points2bbox(pts.reshape(-1,
            # 2 * self.num_points), y_first=False) for pts in
            # pts_coordinate_preds_refine]
            # self.pred_instances.boxsz = torch.cat([get_shape(box.reshape(
            # -1,4)) for box in bbox_pred_refine])
        
        # compute loss
        losses_cls, losses_pts_init, losses_pts_refine, losses_heatmap, \
            losses_offset = multi_apply(
            self.loss_single,
            cls_scores,
        ...
        for i in range(5):
            ...
            gt_lvl_sem_map = F.interpolate(
                gt_sem_map,
                sem_score.shape[-2:]
            ).reshape(-1)
            gt_lvl_sem_weight = F.interpolate(
                gt_sem_weights,
                sem_score.shape[-2:]
            ).reshape(-1)
    ...
    def get_bboxes(
        ...
        for img_id in range(len(img_metas)):
            ...
            proposals = self._get_bboxes_single(
                cls_score_list, bbox_pred_list, hm_scores_list, hm_offsets_list,
    ...
    def _get_bboxes_single(
        ...
        def select(score_map, x, y, ks=2, i=0):
            ...
            x_round = round_func(
                (x / self.point_strides[i]).clamp(
                    min=0,
                    max=score_map.shape[-1] - 1
                )
            )
            y_round = round_func(
                (y / self.point_strides[i]).clamp(
                    min=0,
            ...
            select_indices = indices[
                y_round.to(torch.long), x_round.to(torch.long)]
        ...
        for i_lvl, (cls_score, bbox_pred, points) in enumerate(
            ...
            if self.mask_head:
                inst_ind = torch.arange(
                    cls_score.shape[-2] * cls_score.shape[-1],
                    device=cls_score.devices
                    ) + self.level_sum[i_lvl] + imid_shift[i_lvl]
            ...
            if self.corner_refine:
                if i_lvl > 0:
                    ...
                    x1 = ((x1_new + hm_offset[y1_new.to(torch.long), x1_new.to(
                        torch.long
                    ), 0]) * point_stride).clamp(min=0, max=img_shape[1])
                    y1 = ((y1_new + hm_offset[y1_new.to(torch.long), x1_new.to(
                        torch.long
                    ), 1]) * point_stride).clamp(min=0, max=img_shape[0])
                    x2 = ((x2_new + hm_offset[y2_new.to(torch.long), x2_new.to(
                        torch.long
                    ), 2]) * point_stride).clamp(min=0, max=img_shape[1])
                    y2 = ((y2_new + hm_offset[y2_new.to(torch.long), x2_new.to(
                        torch.long
                    ), 3]) * point_stride).clamp(min=0, max=img_shape[0])
        ...
        if nms:
            if self.mask_head:
                det_bboxes, det_labels, inst_inds, _ = multiclass_nms_rpd(
                ...
                det_bboxes, det_labels, _ = multiclass_nms_rpd(
                    mlvl_bboxes, mlvl_scores,
                    cfg.score_thr, cfg.nms,
                    cfg.max_per_img
                )
    ...
    def forward_train(
        ...
        **kwargs
        ...
        """
        if gt_masks:
            self.masknum_sum = tuple(
                accumulate([0] + [len(mask) for mask in gt_masks][:-1])
            )
        outs = self(x)
        if gt_labels is None:
            loss_inputs = outs + (
            gt_bboxes, gt_sem_map, gt_sem_weights, img_metas)
        else:
            loss_inputs = outs + (
            gt_bboxes, gt_sem_map, gt_sem_weights, gt_labels, img_metas)
        ...
        if self.mask_head:
            self.pred_instances = self.pred_instances[
                self.pred_instances.gt_inds > 0]  # select only foreground labels
            self.pred_instances.gt_inds = self.pred_instances.gt_inds - 1
            mask_loss = self.mask_head(
                x,
                self.pred_instances,
                gt_masks,
                gt_labels
