...
class StageCascadeRPNHead(RPNHead):
    ...
    def anchor_offset(self, anchor_list, anchor_strides, featmap_sizes):
        ...
        def _ctr_offset(anchors, stride, featmap_size):
            ...
            xx = torch.arange(0, feat_w, device=anchors.devices)
            yy = torch.arange(0, feat_h, device=anchors.devices)
        ...
        device = anchor_list[0][0].devices
        num_level_anchors = [anchors.size(0) for anchors in anchor_list[0]]
    ...
    def loss_single(
        self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        if 'max_num' in cfg:
            if 'max_per_img' in cfg:
                assert cfg.max_num == cfg.max_per_img, f'You ' \
                                                       f'set max_num and ' \
                                                       f'max_per_img at the ' \
                                                       f'same time, but get ' \
                                                       f'{cfg.max_num} ' \
                                                       f'and ' \
                                                       f'{cfg.max_per_img} ' \
                                                       f'respectively' \
                                                       'Please delete max_num ' \
                                                       'which will be ' \
                                                       'deprecated.'
            else:
