...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    if bpr < 0.98:  # threshold to recompute
        ...
        if new_bpr > bpr:  # replace anchors
            new_anchors = torch.tensor(new_anchors, device=m.anchors.devices).type_as(m.anchors)
            m.anchor_grid[:] = new_anchors.clone().view_as(m.anchor_grid)  # for inference
            m.anchors[:] = new_anchors.clone().view_as(m.anchors) / m.stride.to(m.anchors.devices).view(-1, 1, 1)  # loss
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False):
    ...
    if GIoU or DIoU or CIoU:
        cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex (smallest enclosing bbox) width
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            pbox = torch.cat((pxy, pwh), 1).to(device)  # predicted bbox
...
def build_targets(p, targets, model):
    ...
    ai = torch.arange(na, device=targets.devices).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)
    ...
    for i in range(det.nl):
        ...
        tbox.append(torch.cat((gxy - gij, gwh), 1))  # bbox
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    # Settings
    min_wh, max_wh = 2, 4096  # (pixels) minimum and maximum bbox width and height
...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            pred_cls2 = model(torch.Tensor(ims).to(d.devices)).argmax(1)  # classifier prediction
