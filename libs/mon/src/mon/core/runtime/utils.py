...
"""A module for CLI configuration parsing utilities.

This module provides utility functions to parse and manage configuration files,
datasets, models, and weights for machine learning projects.
"""
...
def list_config_files(
    ...
    Args:
        ...
        model_root (Path, optional): Root directory of the model. Defaults to None.
        model (str, optional): Model name to filter config files. Defaults to None.
        absolute_path (bool, optional): If True, returns absolute paths. Defaults to False.

    Returns:
        list[Path]: Sorted list of unique configuration file paths.
    """
    def is_valid(x) -> bool:
...
def list_tasks(project_root: Path = None) -> list[str]:
    ...
    Args:
        project_root (Path, optional): Root directory of project. Defaults to None.
...
def list_archs(
    ...
    Args:
        task (str, optional): Task to filter archs. Defaults to None.
        mode (str, optional): Mode of archs ("train" or None). Defaults to None.
        project_root (Path, optional): Root directory of project. Defaults to
            None means list all archs supported in ``mon`` frameworks.

    Returns:
        list[str]: Sorted list of arch names matching task and mode.
    """
...
def list_models(
    ...
    Args:
        task (str, optional): Task to filter models. Defaults to None.
        mode (str, optional): Mode of models ("train" or None). Defaults to None.
        arch (str, optional): Architecture to filter models. Defaults to None.
        project_root (Path, optional): Root directory of project. Defaults to
            None means list all models supported in ``mon`` frameworks.

    Returns:
        list[str]: Sorted list of model names matching task, mode, and arch.
    """
...
def list_weights_files(model: str, project_root: Path = None) -> list[Path]:
    ...
    Args:
        model (str): Model name to filter weights files.
        project_root (Path, optional): Root directory of project. Defaults to None.

    Returns:
        list[Path]: Sorted list of unique weights file paths for the model.
    """
...
def list_datasets(task: str, mode: str, project_root: Path = None) -> list[str]:
    ...
    Args:
        task (str): Task to filter datasets.
        mode (str): Mode of datasets ("train" or None).
        project_root (Path, optional): Root directory of project. Defaults to None.

    Returns:
        list[str]: Sorted list of dataset names matching task and mode.
    """
...
def load_config(config: Any, verbose: bool = True) -> dict | box.Box:
    ...
    Args:
        config (Any): Config file path, dict, or Box object.
        verbose (bool, optional): If True, enables verbose output. Defaults to True.

    Returns:
        dict or box.Box: Loaded configuration as a dictionary or Box object.
    """
    data = None
...
def load_project_defaults(project_root: Path) -> dict:
    ...
    Returns:
        dict: Default configuration of the project.
    """
...
def parse_config_file(config: Path, project_root: Path, model_root: Path = None) -> Path | None:
    ...
    Returns:
        Path: Config file path if found, else None.
    """
...
def parse_model_dir(arch: str, model: str) -> Path | None:
    ...
    Returns:
        Path: Model directory path if found, else None.
    """
...
def parse_model_fullname(name: str, data: str, suffix: str = None) -> str:
    ...
    Args:
        name (str): Model name.
        data (str): BaseDataset name.
        suffix (str, optional): Suffix to append to the model name. Defaults to
            None.

    Returns:
        str: Parsed full model name.
    """
    if not name:
...
def parse_weights_dir(root: Path, weights: Path | Sequence[Path]) -> Path | Sequence[Path]:
    ...
    Args:
        root (Path): Root directory (e.g., project root).
        weights (Path or Sequence[Path]): Weights directory path(s) to parse
            (path or sequence of paths).
    
    Returns:
        Parsed weights path(s) as a single path or a sequence of paths, or None
            if empty.
    """
    root    = Path(root)
...
def parse_weights_file(root: Path, weights: Path | Sequence[Path]) -> Path | Sequence[Path]:
    ...
    Args:
        root (Path): Root directory (e.g., project root).
        weights (Path or Sequence[Path]): Weights file path(s) to parse (path or
            sequence of paths).
...
def parse_weights_from_config(config: Path | dict) -> Path | None:
    ...
    Args:
        config (Path or dict): Config file path or dict.

    Returns:
        Path: Weights file path if found, else None.
    """
    if config is None:
        return None
...
def parse_save_dir(
    ...
    Returns:
        Path: Parsed save directory path.
    """
    save_dir = Path(root)
...
def parse_output_dir(
    ...
    Args:
        root (Path): Root directory. It is assumed to be in this pattern:
            ``root/arch/model/[fullname or dirname]``.
        dirname (Path or str): Directory name (e.g., model name).
        subdir_name (Path or str): Sub-directory name to save outputs.
        src_path (Path or str): Source file path.
        keep_subdirs (bool, optional): If True, keeps subdirectories from
            ``src_path`` relative to ``dirname``. Defaults to False.
        save_nearby (bool, optional): If True, saves outputs nearby the
            ``src_path``. Defaults to False.
    
    Returns:
        Path: Parsed output directory path.
...
def parse_data_dir(root: Path, data_dir: Path = "") -> Path:
    ...
    Args:
        root (Path): Root directory (e.g., project root).
        data_dir (Path, optional): Data directory to parse. Defaults to "".

    Returns:
        Path: Parsed absolute data directory path.
    """
...
def print_run_summary(args: dict | box.Box, full: bool = False):
    ...
    Args:
        args (dict or box.Box): Run configuration arguments.
        full (bool, optional): If True, prints full configuration. Defaults to
            False.
    """
    if full:
