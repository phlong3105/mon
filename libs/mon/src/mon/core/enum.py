...
class Enum(enum.Enum, metaclass=CustomEnumMeta):
    """Extend enum with convenience utilities.

    Add caching, random selection, and conversion helpers for names, indices,
    and values to simplify common enum operations.

    Attributes:
        names (list[Enum]): List of all enum members.
        values (list[Any]): List of all enum member values.
        ints_to_enums (dict[int, Enum]): Mapping from integer indices to enum members.
        strs_to_enums (dict[str, Enum]): Mapping from lowercase names to enum members.
        values_to_enums (dict[Any, Enum]): Mapping from values to enum members.
    """
...
class MLType(Enum):
    ...
    SELF_SUPERVISED = "self_supervised"  # Self-Supervised (or
    # Semi-Supervised) learning with self-generated supervision.
    TEST_TIME = "test_time"  # Test-Time Adaptation (TTA) or Test-Time
    # Augmentation (TTA).
...
class DepthSource(Enum):
    ...
    DAAC_ViTS = "depth_daac_vits"  # Depth Anything at Any Condition with
    # ViT-S encoder
