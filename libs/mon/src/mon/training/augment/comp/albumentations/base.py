...
class Compose(Compose_):
    ...
    def __init__(self, transforms: list[Any], *args, **kwargs):
        """Initialize a new instance.

        Args:
            transforms (list[Any]): A list of transformation operations.
                If any element in ``transforms`` is a dict, it will be used to
                build the corresponding transformation operation.
        """
...
def build_transforms(transforms: list[Any]) -> list[BasicTransform]:
    ...
    Returns:
       list[BasicTransform]: A list of built transformation operations.
    ...
    if len(transform_ops) == 0:
        raise ValueError(
            f"'transforms' must contain at least one valid transformation."
        )
