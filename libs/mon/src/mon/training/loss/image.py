...
class ExposureControlLoss(BaseLoss):
    ...
    def __init__(
        ...
        Args:
            ...
            mean_val (float): Well-exposedness level E. Defaults to 0.6.
            required_grad (bool): If True, ``mean_val`` is learnable. Defaults to True.
...
class PSNRLoss(BaseLoss):
    ...
    def __init__(self, to_y: bool = False, reduction: str = "mean"):
        ...
        Args:
            to_y (bool): If True, convert RGB to Y-channel before computing PSNR.
                Defaults to False.
            reduction (str): Reduction method to apply to the loss. One of:
                ["mean", "sum", "none"]. Defaults to "mean".
        """
...
class SpatialConsistencyLoss(BaseLoss):
    ...
    def __init__(
        ...
        Args:
            num_regions (int): Number of regions to consider for gradient
                comparison. One of: [4, 8, 16, 24]. Defaults to 4.
            patch_size (int): Patch size for local patch pooling. Defaults to 4.
            reduction (str): Reduction method to apply to the loss. One of:
                ["mean", "sum", "none"]. Defaults to "mean".
        """
        super().__init__(reduction=reduction)
