...
This module provides various layers, blocks, and modules for the TensorMOG model.
"""
...
class MOG:
    """Mixture of Gaussians for background subtraction."""
    ...
    def __init__(
        ...
        Args:
            ...
            num_gaussians (int, optional): Number of Gaussian components per
                pixel. Defaults to 3.
            learning_rate (float, optional): Rate at which model parameters are
                updated. Defaults to 0.02.
            matching_thres (float, optional): Threshold for matching a pixel to
                a Gaussian. Defaults to 4.0 (2 squared).
            background_thres (float, optional): Threshold for background
                classification. Defaults to 0.6.
        ...
        self.mix_mu = torch.zeros((1, 3, height, width, num_gaussians), device=device)
        self.mix_var = torch.zeros((1, 1, height, width, num_gaussians), device=device)
        self.mix_weight = torch.zeros((1, 1, height, width, num_gaussians), device=device)
        self.mix_key = torch.zeros((1, 1, height, width, num_gaussians), device=device)

    # --- Callable & Context Manager ---
    def update(self, image: Tensor):
    ...
    def get_background(self) -> Tensor:
        """Generate the background image based on the model.

        Returns:
            Tensor: background image of shape (1, 3, height, width) and values
    ...
    def get_foreground(self, image: Tensor, background: Tensor) -> Tensor:
        """Generate a foreground mask by comparing the image to the background.

        Args:
            image (Tensor): Input image of shape (1, 3, H, W) and values ranging
                from 0.0 to 1.0.
            background (Tensor): Background image of shape (1, 3, H, W) and
                values ranging from 0.0 to 1.0.

        Returns:
            Tensor: Foreground mask of shape (1, 1, H, W) with values of 0.0 for
                the background and 1.0 for foreground.
        """
        diff = torch.abs(image - background)                          # [1, 3, H, W]
        mask = torch.any(diff > (80.0 / 255.0), dim=1, keepdim=True)  # [1, 1, H, W]
        return mask.float()
    ...
    def estimate_entropy_mask(self) -> Tensor:
    ...
    def estimate_noise_ratio(self) -> float:
        ...
        return torch.sum(self.estimate_entropy_mask()).item() / (self.height * self.width)

    # --- Utilities ---
    def _one_hot(self, indices: Tensor, depth: int) -> Tensor:
...
class HVR:
    ...
    Manages a Mixture of Gaussians (MOG) model with state-based updates to
    handle background stability and anomalies.
    """
    ...
    def __init__(
        ...
        self.counter = 0
        self.tau = 0.0
        self.bg = torch.zeros((1, 3, height, width), device=device)
        self.bg_model = MOG(
            height=height,
            width=width,
    ...
    def update(self, image: Tensor):
        ...
        self.bg_model.update(image)
        entropy = self.bg_model.estimate_entropy()
    ...
    def _handle_update_state(self, is_anomaly: bool):
        ...
        if is_anomaly:
            ...
            if self.counter >= self.num_updates:
                ...
                self.counter = 0
                self.bg = self.bg_model.get_background()
                self.bg_model.learning_rate = max(self.bg_model.learning_rate / 2, 0.001)
        else:
            self.counter = 0
    ...
    def _handle_suspense_state(self, is_anomaly: bool):
        ...
        if not is_anomaly:
            ...
            if self.counter >= self.num_updates:
                self.state = self.State.UPDATE
                self.counter = 0
                self.bg_model.learning_rate = min(self.bg_model.learning_rate * 2, 0.1)
        else:
            self.counter = 0
    ...
    def get_foreground(self, image: Tensor) -> Tensor:
        """Generate a foreground mask by comparing the image to the background."""
        return self.bg_model.get_foreground(image, self.bg)

    def estimate_entropy(self) -> float:
        """Estimate the model's uncertainty."""
        return self.bg_model.estimate_entropy()
