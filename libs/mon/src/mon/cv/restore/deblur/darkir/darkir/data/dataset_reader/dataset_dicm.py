...
def main_dataset_dicm(rank = 1,
                         test_path='../../data/datasets/DICM',
                         batch_size_test=1, 
                         verbose=False, 
                         num_workers=1, 
                         crop_type='Random',
    ...
    paths_blur_valid = [os.path.join(PATH_VALID, path) for path in os.listdir(PATH_VALID)]
    paths_sharp_valid = [os.path.join(PATH_VALID, path) for path in os.listdir(PATH_VALID)]        
...
if __name__ == '__main__':
    test_loader, samplers = main_dataset_dicm(test_path='/mnt/valab-datasets/Low_Light_Enhancement_Datasets/DICM/DICM')
