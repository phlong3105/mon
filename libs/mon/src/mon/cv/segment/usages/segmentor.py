...
class SAMBoxSegmentor(BoxSegmentor):
    ...
    def process_dir(
        ...
        """
        image_dir  = Path(image_dir).normalize(exist=True)
        label_dir  = Path(label_dir).normalize(exist=True)
        ...
        # Process each pair of image and label file
        with create_progress_bar() as pbar:
            for i, image_file in pbar.track(
                ...
                masks = self.process(image=image, bbox=bbox)
