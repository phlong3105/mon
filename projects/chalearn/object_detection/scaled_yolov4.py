...
train_configs = {
    ...
    "yolov4-p7_chalearnltdmonth_1280": {
        # "weights"     : os.path.join(yolov4_pretrained_dir, "yolov4-p7_coco.pt"),
        "weights"     : os.path.join(pretrained_dir,  "scaled_yolov4", "exp0_yolov4-p7_chalearnltdmonth_1280", "weights", "last.pt"),
        "cfg"         : os.path.join(yolov4_root_dir, "models", "yolov4-p7.yaml"),
        "data"        : os.path.join(current_dir,     "chalearnltdmonth.yaml"),
        "hyp"         : os.path.join(yolov4_root_dir, "data", "hyp.scratch.yaml"),
        "epochs"      : 100,
...
conf_weights = {
    ...
    "dw_act": {'Apr': 0.1, 'Aug': 0.1, 'Jan': 0.2639670199496633, 'Jul': 0.1, 'Jun': 0.10187234973704269, 'Mar': 0.10231222536205314, 'May': 0.1, 'Sep': 0.4878866331464872}
}
...
def create_pkl_from_txts(
    ...
    """
    total_images = len(glob.glob(os.path.join(data_dir, "chalearn", "ltd", "val", "**", "*.jpg"), recursive=True))
    ...
    with progress_bar() as pbar:
        for file in pbar.track(
            ...
            if day not in predictions[month]:
                predictions[month][day] = {"boxes": [], "labels": [], "scores": []}
            if isinstance(conf_thres, dict):
                class_conf_thres = conf_thres[month]
            else:
                class_conf_thres = conf_thres
            if not isinstance(class_conf_thres, (list, tuple)):
                class_conf_thres = [class_conf_thres for _ in range(4)]
                ...
                with open(file, "r") as d:
                    ...
                    for row in reader:
                        ...
                        if conf >= class_conf_thres[cls]:
                            ...
                            predictions[month][day]["scores"].append(conf)
