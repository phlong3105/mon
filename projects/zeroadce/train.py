...
def parse_args():
    ...
    parser.add_argument("--cfg",         type=str,            default="zeroadce", help="The training cfg to use.")
    parser.add_argument("--project",     type=str,            default="project",  help="Save results to root/project/name")
    parser.add_argument("--weights",     type=str,            default=None,       help="Weights path.")
    parser.add_argument("--batch-size",  type=int,            batch_size=8,       help="Total Batch size for all GPUs.")
    parser.add_argument("--img-size",    type=int, nargs="+", img_size=None,      help="Image sizes.")
    parser.add_argument("--accelerator", type=str,            accelerator="auto", help="Supports passing different accelerator types ('cpu', 'gpu', 'tpu', 'ipu', 'hpu', 'mps', 'auto') as well as custom accelerator instances.")
    parser.add_argument("--devices",     type=str,            devices=1,          help="Will be mapped to either gpus, tpu_cores, num_processes or ipus based on the accelerator type.")
    parser.add_argument("--max-epochs",  type=int,            max_epochs=200,     help="Stop training once this number of epochs is reached.")
    parser.add_argument("--strategy",    type=str,            strategy=None,      help="Supports different training strategies with aliases as well custom strategies.")
...
if __name__ == "__main__":
    ...
    project     = input_args.get("project",     None) or module.model["project"]
    weights     = input_args.get("weights",     None) or module.model["pretrained"]
    batch_size  = input_args.get("batch_size",  None) or module.data["batch_size"]
    shape       = input_args.get("img_size",    None) or module.data["shape"]
    accelerator = input_args.get("accelerator", None) or module.trainer["accelerator"]
    devices     = input_args.get("devices",     None) or module.trainer["devices"]
    max_epochs  = input_args.get("max_epochs",  None) or module.trainer["max_epochs"]
    strategy    = input_args.get("strategy",    None) or module.trainer["strategy"]
